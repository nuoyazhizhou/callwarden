...
pub fn cas_global_lookup<'py>(
        ...
        .map_err(|e| PyIOError::new_err(format!("fetch 失败: {}", e)))?
    {
        ...
        dict.set_item("cas_key", row.get::<_, String>(0).map_err(|e| PyIOError::new_err(format!("字段读取失败 cas_key: {}", e)))?)?;
        dict.set_item("content_hash", row.get::<_, String>(1).map_err(|e| PyIOError::new_err(format!("字段读取失败 content_hash: {}", e)))?)?;
        dict.set_item("language", row.get::<_, String>(2).map_err(|e| PyIOError::new_err(format!("字段读取失败 language: {}", e)))?)?;
        dict.set_item("file_size", row.get::<_, i64>(3).map_err(|e| PyIOError::new_err(format!("字段读取失败 file_size: {}", e)))?)?;
        dict.set_item("total_lines", row.get::<_, i64>(4).map_err(|e| PyIOError::new_err(format!("字段读取失败 total_lines: {}", e)))?)?;
        dict.set_item("parser_version", row.get::<_, String>(5).map_err(|e| PyIOError::new_err(format!("字段读取失败 parser_version: {}", e)))?)?;
        dict.set_item("callwarden_version", row.get::<_, String>(6).map_err(|e| PyIOError::new_err(format!("字段读取失败 callwarden_version: {}", e)))?)?;
        dict.set_item("extraction_config_version", row.get::<_, String>(7).map_err(|e| PyIOError::new_err(format!("字段读取失败 extraction_config_version: {}", e)))?)?;
        dict.set_item("abi_version", row.get::<_, String>(8).map_err(|e| PyIOError::new_err(format!("字段读取失败 abi_version: {}", e)))?)?;
        dict.set_item("input_abi_version", row.get::<_, String>(9).map_err(|e| PyIOError::new_err(format!("字段读取失败 input_abi_version: {}", e)))?)?;
        dict.set_item("state", row.get::<_, String>(10).map_err(|e| PyIOError::new_err(format!("字段读取失败 state: {}", e)))?)?;
        dict.set_item("parsed_at", row.get::<_, f64>(11).map_err(|e| PyIOError::new_err(format!("字段读取失败 parsed_at: {}", e)))?)?;
...
pub fn cas_global_get_state(db_path: &str, cas_key: &str) -> PyResult<Option<String>> {
    ...
    if let Some(row) = rows
        ...
        .map_err(|e| PyIOError::new_err(format!("fetch 失败: {}", e)))?
    {
        Ok(Some(row.get::<_, String>(0).map_err(|e| PyIOError::new_err(format!("字段读取失败: {}", e)))?))
    } else {
        Ok(None)
    }
}
...
pub fn cas_local_get_file_generation<'py>(
        ...
        .map_err(|e| PyIOError::new_err(format!("fetch 失败: {}", e)))?
    {
        ...
        dict.set_item("latest_session_id", row.get::<_, String>(2).map_err(|e| PyIOError::new_err(format!("字段读取失败 latest_session_id: {}", e)))?)?;
        dict.set_item("latest_session_epoch", row.get::<_, i64>(3).map_err(|e| PyIOError::new_err(format!("字段读取失败 latest_session_epoch: {}", e)))?)?;
        dict.set_item("latest_seq", row.get::<_, i64>(4).map_err(|e| PyIOError::new_err(format!("字段读取失败 latest_seq: {}", e)))?)?;
        dict.set_item("latest_seen_generation", row.get::<_, String>(5).map_err(|e| PyIOError::new_err(format!("字段读取失败 latest_seen_generation: {}", e)))?)?;
        dict.set_item("latest_committed_generation", row.get::<_, String>(6).map_err(|e| PyIOError::new_err(format!("字段读取失败 latest_committed_generation: {}", e)))?)?;
        Ok(Some(dict.into_any()))
    } else {
        Ok(None)
    }
