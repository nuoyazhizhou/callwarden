...
impl ToolchainStore {
    ...
    pub fn delete_toolchain(&self, name_or_id: &str) -> Result<u64, rusqlite::Error> {
        ...
        if let Some(id) = id {
            ...
            let affected = conn.execute(
                "DELETE FROM toolchains WHERE id = ?1",
                params![id],
            )?;
            Ok(affected as u64)
        } else {
    ...
    pub fn get_resolved_edges(
        ...
        let iter: Box<dyn Iterator<Item = rusqlite::Result<Value>>> = if let Some(csid) = caller_symbol_id {
    ...
    pub fn attach_to(&self, target_conn: &Connection, schema_name: &str) -> Result<(), rusqlite::Error> {
...
pub fn compute_build_context_hash(
    ...
    let flags_str: String = sorted_flags
        .iter()
        .map(|f| format!("{};", f))
        .collect();
    let defines_str: String = sorted_defines
        ...
        .map(|(k, v)| format!("{}={};", k, v))
        .collect();
    let includes_str: String = norm_includes
        .iter()
        .map(|p| format!("{};", p))
        .collect();
...
fn toolchain_row_to_json(row: &Row<'_>) -> rusqlite::Result<Value> {
    ...
    let include_dirs: Value = serde_json::from_str(&include_dirs_str).unwrap_or(Value::Array(vec![]));
    let predefined_macros: Value = serde_json::from_str(&predefined_macros_str).unwrap_or(Value::Object(Map::new()));
    ...
    m.insert("created_at".to_string(), Value::Number(serde_json::Number::from_f64(created_at).unwrap_or_else(|| 0.into())));
    m.insert("updated_at".to_string(), Value::Number(serde_json::Number::from_f64(updated_at).unwrap_or_else(|| 0.into())));
...
fn build_context_row_to_json(row: &Row<'_>) -> rusqlite::Result<Value> {
    ...
    let compile_flags: Value = serde_json::from_str(&compile_flags_str).unwrap_or(Value::Array(vec![]));
    let defines: Value = serde_json::from_str(&defines_str).unwrap_or(Value::Object(Map::new()));
    let include_paths: Value = serde_json::from_str(&include_paths_str).unwrap_or(Value::Array(vec![]));

    let mut m = Map::new();
    m.insert("workspace_id".to_string(), Value::Number(workspace_id.into()));
    m.insert("build_context_hash".to_string(), Value::String(build_context_hash));
    m.insert("name".to_string(), Value::String(name));
...
fn build_context_to_json(
    ...
    let defines: Value = {
        let map: std::collections::HashMap<String, String> = defines
            .iter()
            .cloned()
            .collect();
    ...
    let mut m = Map::new();
    m.insert("workspace_id".to_string(), Value::Number(workspace_id.into()));
    m.insert("build_context_hash".to_string(), Value::String(build_context_hash.to_string()));
...
fn resolved_edge_row_to_json(row: &Row<'_>) -> rusqlite::Result<Value> {
    ...
    m.insert("workspace_id".to_string(), Value::Number(workspace_id.into()));
    m.insert("build_context_hash".to_string(), Value::String(build_context_hash));
    m.insert("caller_symbol_id".to_string(), Value::Number(caller_symbol_id.into()));
    m.insert("callee_symbol_id".to_string(), Value::Number(callee_symbol_id.into()));
    ...
    m.insert("resolution_method".to_string(), Value::String(resolution_method));
    m.insert("created_at".to_string(), Value::Number(serde_json::Number::from_f64(created_at).unwrap_or_else(|| 0.into())));
    Ok(Value::Object(m))
}
...
mod tests {
    ...
    fn test_open_in_memory_initializes_schema() {
        ...
        // 验证 4 张表存在
        for table in ["toolchains", "workspace_toolchains", "workspace_build_contexts", "resolved_edges"] {
            let count: i64 = conn
                .query_row(
    ...
    fn test_register_toolchain_dedup_by_fingerprint() {
        ...
        let store = ToolchainStore::open_in_memory().unwrap();
        let _ = store
            .register_toolchain("gcc-13", "/usr/bin/gcc", "gcc", "13.0", "", "", &[], &[], "fp_dedup", "")
            .unwrap();
        // 用不同 name 但相同 fingerprint 注册
        let result = store
            .register_toolchain("gcc-13-alias", "/usr/bin/gcc", "gcc", "13.0", "", "", &[], &[], "fp_dedup", "")
            .unwrap();
    ...
    fn test_get_toolchain_by_name_and_id() {
        ...
        let registered = store
            .register_toolchain("clang-18", "/usr/bin/clang", "clang", "18.0", "", "", &[], &[], "fp_clang", "")
            .unwrap();
    ...
    fn test_list_toolchains_returns_all_ordered_by_id() {
        let store = ToolchainStore::open_in_memory().unwrap();
        store.register_toolchain("tc1", "/p1", "gcc", "", "", "", &[], &[], "fp1", "").unwrap();
        store.register_toolchain("tc2", "/p2", "clang", "", "", "", &[], &[], "fp2", "").unwrap();
        store.register_toolchain("tc3", "/p3", "arm-gcc", "", "", "", &[], &[], "fp3", "").unwrap();
        let all = store.list_toolchains().unwrap();
        assert_eq!(all.len(), 3);
        // 验证按 id 升序
    ...
    fn test_delete_toolchain_by_id() {
        let store = ToolchainStore::open_in_memory().unwrap();
        let registered = store.register_toolchain("tc1", "/p1", "gcc", "", "", "", &[], &[], "fp1", "").unwrap();
        let id = registered["id"].as_i64().unwrap();
    ...
    fn test_delete_toolchain_cascades_bindings() {
        ...
        let store = ToolchainStore::open_in_memory().unwrap();
        let tc = store.register_toolchain("tc1", "/p1", "gcc", "", "", "", &[], &[], "fp1", "").unwrap();
    ...
    fn test_get_workspace_toolchains_filters_by_build_context() {
        ...
        let tc = store.register_toolchain("tc1", "/p1", "gcc", "", "", "", &[], &[], "fp1", "").unwrap();
        let tc_id = tc["id"].as_i64().unwrap();
        store.bind_toolchain_to_workspace(1, tc_id, "bch_debug").unwrap();
        store.bind_toolchain_to_workspace(1, tc_id, "bch_release").unwrap();
        let debug = store.get_workspace_toolchains(1, Some("bch_debug")).unwrap();
        assert_eq!(debug.len(), 1);
        let release = store.get_workspace_toolchains(1, Some("bch_release")).unwrap();
    ...
    fn test_register_build_context_set_active_clears_others() {
        ...
        let store = ToolchainStore::open_in_memory().unwrap();
        store.register_build_context(1, "debug", &["-g".into()], &[], &[], true).unwrap();
        store.register_build_context(1, "release", &["-O2".into()], &[], &[], true).unwrap();
        // 现在 release 应为 active，debug 应为非 active
        let contexts = store.list_build_contexts(1).unwrap();
        assert_eq!(contexts.len(), 2);
        let active_count = contexts.iter().filter(|c| c["is_active"].as_bool().unwrap_or(false)).count();
    ...
    fn test_list_build_contexts() {
        ...
        store.register_build_context(1, "debug", &["-g".into()], &[], &[], false).unwrap();
        store.register_build_context(1, "release", &["-O2".into()], &[], &[], true).unwrap();
        let contexts = store.list_build_contexts(1).unwrap();
        assert_eq!(contexts.len(), 2);
        // active 应排在前
        assert_eq!(contexts[0]["name"], "release");
        assert_eq!(contexts[0]["is_active"], true);
    }
    ...
    fn test_set_active_build_context() {
        let store = ToolchainStore::open_in_memory().unwrap();
        // 使用不同的 compile_flags 产生不同的 build_context_hash（避免 INSERT OR REPLACE 覆盖）
        let c1 = store.register_build_context(1, "debug", &["-g".into()], &[], &[], false).unwrap();
        let c2 = store.register_build_context(1, "release", &["-O2".into()], &[], &[], false).unwrap();
        // 初始都非 active
        // 设置 c1 为 active
    ...
    fn test_delete_build_context_cascades_edges() {
        ...
        let store = ToolchainStore::open_in_memory().unwrap();
        let ctx = store.register_build_context(1, "debug", &[], &[], &[], false).unwrap();
        let bch = ctx["build_context_hash"].as_str().unwrap();
        store.store_resolved_edges(
            1,
            bch,
            &[
                ResolvedEdgeInput {
    ...
    fn test_store_resolved_edges_inserts_rows() {
        let store = ToolchainStore::open_in_memory().unwrap();
        let inserted = store.store_resolved_edges(
            1,
            "bch1",
            &[
                ResolvedEdgeInput {
    ...
    fn test_store_resolved_edges_dedup_by_unique_constraint() {
        ...
        store.store_resolved_edges(1, "bch1", &[edge.clone()]).unwrap();
        ...
        store.store_resolved_edges(
            ...
            &[
                ResolvedEdgeInput {
                    ...
                    callee_name: "foo".into(), callee_file: "a.rs".into(),
                    call_line: 10, resolution_method: "exact_match".into(),
                },
                ResolvedEdgeInput {
                    caller_symbol_id: 5, callee_symbol_id: 2,
                    callee_name: "foo".into(), callee_file: "a.rs".into(),
                    call_line: 20, resolution_method: "exact_match".into(),
                },
            ],
        ).unwrap();
    ...
    fn test_delete_resolved_edges_by_context() {
        let store = ToolchainStore::open_in_memory().unwrap();
        store.store_resolved_edges(1, "bch1", &[
            ResolvedEdgeInput {
                caller_symbol_id: 1, callee_symbol_id: 2,
                callee_name: "foo".into(), callee_file: "a.rs".into(),
                call_line: 10, resolution_method: "exact_match".into(),
        ...
        store.store_resolved_edges(1, "bch2", &[
            ResolvedEdgeInput {
                caller_symbol_id: 1, callee_symbol_id: 3,
                callee_name: "bar".into(), callee_file: "b.rs".into(),
                call_line: 20, resolution_method: "exact_match".into(),
            },
        ]).unwrap();
        // 只删 bch1
    ...
    fn test_list_build_context_edges() {
        ...
        store.store_resolved_edges(1, "bch2", &[
            ...
            ResolvedEdgeInput {
                caller_symbol_id: 1, callee_symbol_id: 4,
                callee_name: "baz".into(), callee_file: "c.rs".into(),
                call_line: 30, resolution_method: "exact_match".into(),
            },
        ]).unwrap();
    ...
    fn test_resolve_toolchain_fallback_to_active() {
        ...
        let tc = store.register_toolchain("tc1", "/p1", "gcc", "", "", "", &[], &[], "fp1", "").unwrap();
        let tc_id = tc["id"].as_i64().unwrap();
        let ctx = store.register_build_context(1, "debug", &[], &[], &[], true).unwrap();
        let active_bch = ctx["build_context_hash"].as_str().unwrap();
        store.bind_toolchain_to_workspace(1, tc_id, active_bch).unwrap();
        // 用一个不存在的 bch 查询，应降级到 active
        let resolved = store.resolve_toolchain(1, Some("nonexistent_bch")).unwrap().unwrap();
        assert_eq!(resolved["name"], "tc1");
    }
    ...
    fn test_compute_build_context_hash_deterministic() {
        // 相同输入应产生相同 hash
        let h1 = compute_build_context_hash(&["-g".into(), "-O0".into()], &[("DEBUG".into(), "1".into())], &["/usr/include".into()]);
        let h2 = compute_build_context_hash(&["-O0".into(), "-g".into()], &[("DEBUG".into(), "1".into())], &["/usr/include".into()]);
    ...
    fn test_attach_to_target_connection() {
        ...
        let count: i64 = target
            .query_row("SELECT COUNT(*) FROM toolchain.toolchains", [], |row| row.get(0))
            .unwrap();
        assert_eq!(count, 1);
    }
}
