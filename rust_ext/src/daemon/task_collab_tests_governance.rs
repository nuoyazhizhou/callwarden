...
#[test]
fn begin_immediate_retries_only_before_writer_transaction_is_acquired() {
    ...
    let blocker = thread::spawn(move || {
        ...
        conn.execute_batch(
            "PRAGMA journal_mode=WAL;
             CREATE TABLE IF NOT EXISTS busy_retry_probe (id INTEGER PRIMARY KEY);
             BEGIN IMMEDIATE;",
        )
        .unwrap();
    ...
    let mut contender = Connection::open(&db_path).unwrap();
    contender.busy_timeout(Duration::from_millis(25)).unwrap();
    let transaction = begin_immediate_with_retry(&mut contender, "busy-retry-test").unwrap();
    transaction.commit().unwrap();
    blocker.join().unwrap();
}

#[test]
fn begin_immediate_returns_after_bounded_busy_retries() {
    let (_dir, db_path) = temp_db();
    let (locked_tx, locked_rx) = mpsc::channel();
    let blocker_path = db_path.clone();
    let blocker = thread::spawn(move || {
    ...
    let error = begin_immediate_with_retry(&contender, "bounded-busy-test").unwrap_err();
    assert_eq!(error.code, "internal_error");
    assert!(error.message.contains("开启 bounded-busy-test 事务失败"));
    blocker.join().unwrap();
}
...
fn test_governance_projection_returns_diagnosis_without_lease_token() {
    ...
    let proj = store
        .handle_task_governance_projection_get(peer, &serde_json::json!({"task_id": "T-G3-PROJ"}))
        .unwrap();
...
fn test_verdict_submit_appends_replays_and_rejects_conflicts() {
    ...
    let count: i64 = conn
        ...
        .unwrap();
    assert_eq!(count, 1);
    let (task_hash, persisted_step_id, reviewer_provenance): (String, String, String) = conn
        .query_row(
            "SELECT contract_hash, step_id, reviewer_identity FROM task_verdict_events
    ...
    // P0C-R4 回归：verdict.submit 必须持久化 step_id，否则 reviewer_pass handoff 因
    // source_step_id == verdict.step_id 恒失败（E_HANDOFF_VERDICT_PROVENANCE_MISMATCH）。
    assert_eq!(persisted_step_id, step_id, "verdict.submit 必须持久化绑定 step_id");
    assert!(reviewer_provenance.contains("role_contract"));
    assert!(reviewer_provenance.contains("verdict-native-request-1"));
}

#[test]
fn test_verdict_submit_new_schema_role_contract_triple() {
    let (_dir, db_path) = temp_db();
    let store = TaskCollabStore::new(&db_path)
        .unwrap()
        .with_clock(Arc::new(AuthoritativeClock::new()));
    ...
    let (step_id, role_contract_id, role_contract_revision, role_contract_revision_id, legacy_role_contract_id, role_contract_hash) = {
        let conn = store.conn.lock().unwrap();
        conn.execute(
            "UPDATE tasks SET status = 'review' WHERE id = 'T-VERDICT-NEWSCHEMA'",
            [],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO task_contract_revisions
                 (contract_id, revision, contract_hash, profile, task_id, workspace_id,
                  envelope_payload, created_at, created_by)
                 VALUES ('TC-VERDICT-NS', 1, 'sha256:task-contract', 'review',
                         'T-VERDICT-NEWSCHEMA', 1, '{}', 1.0, 'test')",
            [],
        )
        .unwrap();
        let step_id: String = conn
            .query_row(
                "SELECT id FROM task_steps WHERE task_id = 'T-VERDICT-NEWSCHEMA'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        let row: (String, String, i64, String, String) = conn
            .query_row(
                "SELECT l.role_contract_lineage_id, r.role_contract_revision_id, r.revision,
                        r.role_contract_hash, legacy.contract_id
                 FROM role_contract_lineages l
                 JOIN role_contract_revisions r
                   ON r.role_contract_lineage_id = l.role_contract_lineage_id
                 JOIN role_contracts legacy
                   ON legacy.task_id = l.task_id AND legacy.role = l.role
                  AND legacy.revision = r.revision AND legacy.is_current = 1
                 WHERE l.task_id = 'T-VERDICT-NEWSCHEMA'
                   AND l.role = 'reviewer'
                   AND r.revision = 1",
                [],
            ...
            .unwrap();
        (step_id, row.0, row.2, row.1, row.4, row.3)
    };
    ...
    register_agent_with_identity(
        ...
        &peer,
        "agent-ns-reviewer",
        "agent-ns-reviewer-inst",
        "agent-ns-reviewer-sess",
        "reviewer",
    );
    let identity = lease_identity(
        "agent-ns-reviewer",
    ...
    let verdict_params = serde_json::json!({
        "task_id": "T-VERDICT-NEWSCHEMA",
        "step_id": step_id,
        "verdict_id": "V-NEWSCHEMA-1",
        "contract_id": "TC-VERDICT-NS",
        "contract_revision": 1,
        "contract_hash": "sha256:task-contract",
        "role_contract_id": role_contract_id,
        "role_contract_revision": role_contract_revision,
        ...
        "overall": "pass",
        "attestation": "reviewed independently",
        "request_id": "verdict-new-schema-request-1",
        "identity": identity,
        "lease_token": lease["token"],
        "fencing_counter": lease["fencing_counter"],
    });
    let first = store
        .handle_verdict_submit(peer.clone(), &verdict_params)
        .unwrap();
    assert_eq!(first["verdict_id"], "V-NEWSCHEMA-1");
    assert_eq!(first["replayed"], false);

    // The governance projection also exposes the legacy reviewer contract
    // alias. It must resolve to the same canonical lineage/hash, rather than
    // being checked against a different legacy payload hash.
    let mut legacy_alias = verdict_params.clone();
    legacy_alias["verdict_id"] = Value::String("V-NEWSCHEMA-ALIAS".to_string());
    legacy_alias["request_id"] = Value::String("verdict-new-schema-alias".to_string());
    legacy_alias["role_contract_id"] = Value::String(legacy_role_contract_id);
    let alias = store
        .handle_verdict_submit(peer.clone(), &legacy_alias)
        .expect("legacy reviewer contract alias must resolve to canonical lineage");
    assert_eq!(alias["verdict_id"], "V-NEWSCHEMA-ALIAS");
    assert_eq!(alias["replayed"], false);

    let mut revision_id_params = verdict_params.clone();
    revision_id_params["verdict_id"] = Value::String("V-NEWSCHEMA-REVISION".to_string());
    revision_id_params["request_id"] = Value::String("verdict-new-schema-revision".to_string());
    revision_id_params["role_contract_id"] = Value::String(role_contract_revision_id);
    let revision = store
        .handle_verdict_submit(peer.clone(), &revision_id_params)
        .expect("role contract revision id must resolve to canonical lineage");
    assert_eq!(revision["verdict_id"], "V-NEWSCHEMA-REVISION");
    assert_eq!(revision["replayed"], false);

    let mut wrong_role_hash = verdict_params.clone();
    wrong_role_hash["request_id"] = Value::String("verdict-new-schema-request-2".to_string());
    wrong_role_hash["verdict_id"] = Value::String("V-NEWSCHEMA-2".to_string());
    wrong_role_hash["role_contract_hash"] = Value::String("sha256:wrong".to_string());
    let err = store
        .handle_verdict_submit(peer, &wrong_role_hash)
        .unwrap_err();
    ...
    let count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM task_verdict_events WHERE task_id = 'T-VERDICT-NEWSCHEMA'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    assert_eq!(count, 3);
    let (task_contract_id, rc_lineage_id, rc_hash): (String, String, String) = conn
        .query_row(
            "SELECT contract_id, role_contract_lineage_id, role_contract_hash
                 FROM task_verdict_events WHERE verdict_id = 'V-NEWSCHEMA-1'",
            [],
            |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)),
        )
        .unwrap();
    assert_eq!(task_contract_id, "TC-VERDICT-NS");
    assert_eq!(rc_lineage_id, role_contract_id);
    assert_eq!(rc_hash, role_contract_hash);
}
