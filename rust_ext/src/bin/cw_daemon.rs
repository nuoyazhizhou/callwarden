...
mod unix {
    ...
    fn handle_reload(config_path: &Option<PathBuf>, current_config: &DaemonConfig) {
        ...
        if new_config.toolchain_db_path != current_config.toolchain_db_path {
            eprintln!(
                "[cw_daemon] [WARN] SIGHUP reload: toolchain_db_path 变更 ({} → {})，需重启生效",
                current_config.toolchain_db_path.display(),
                new_config.toolchain_db_path.display()
            );
        }
        ...
        if new_config.socket_path == current_config.socket_path
            ...
            && new_config.toolchain_db_path == current_config.toolchain_db_path
        {
