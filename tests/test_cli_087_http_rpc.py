...
1. test_success    —— task.resolve_quality_finding {finding_id, resolution, identity}
                      经 thin client 直达 daemon，断言返回结构化结果且无 "error"（happy path）。
                      需 Reviewer 在 active workspace 播种一条 open finding 并提供合法
                      identity（见 LEGACY_IDENTITY / CW_TEST_FINDING_ID）。
2. test_invalid    —— task.resolve_quality_finding {}（缺 finding_id）：断言含 "error"。
3. test_authority  —— 不带 identity 调用：daemon 在写操作前拒绝，
...
注意：CLI-087 的 Python handler `_local_resolve_finding` 已改为 fail-closed：
local fallback 直接 raise DaemonUnavailableError，不再调用 db.resolve_task_quality_finding。
本模块只验证「thin client 把请求正确路由到 daemon 并获得结构化响应」，
业务 authority 为 Rust daemon（task_collab.rs::handle_task_resolve_quality_finding）。
...
def _check_success(client):
    """test_success 的可复用断言逻辑：resolve 写操作直达 daemon，结构化成功。"""
    params = {
    ...
    result = client.call("task.resolve_quality_finding", params)
    assert isinstance(result, dict), f"result 应为 dict，实为 {type(result)}"
    assert "error" not in result, f"resolve 不应含 error（happy path）：{result}"
    return True
...
def _check_unavailable(dead_client):
    ...
    try:
        r = dead_client.call("task.resolve_quality_finding", {
            "finding_id": FINDING_ID,
            "resolution": RESOLUTION,
            "resolved_by": "cli-087-test",
        })
