"""pytest 全局配置：测试数据库隔离（防止污染 ~/.callwarden/）

问题背景：
    CodeGraphDB(workspace_root=tmp) 不传 db_path 时，__init__ 会调用
    get_project_db_path(workspace_root) 在 ~/.callwarden/<hash>/ 下建库。
    pytest 的 tmp fixture 每次运行路径带随机后缀，路径不同 → hash 不同 →
    每次跑测试就在 ~/.callwarden/ 下新建一批孤儿目录，用完不清理。

方案：
    autouse fixture 在测试期间 monkey-patch get_project_db_path，
    让它返回 tmp_path 下的路径，确保测试数据库完全隔离在临时目录中，
    测试结束自动回收，不污染 ~/.callwarden/。
...
def _isolate_db_path(tmp_path, monkeypatch):
    """自动隔离测试数据库路径，避免污染 ~/.callwarden/

    所有测试自动生效，无需在测试代码中显式引用。
    patch 后 CodeGraphDB(workspace_root=tmp) 的数据库会建在
    tmp_path/test_isolated.db，而非 ~/.callwarden/<hash>/callwarden.db。
    """
