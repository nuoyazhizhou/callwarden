...
def _build_gate_report(
    ...
    for gate_file, gate_name in _GATE_TEST_FILES.items():
        ...
        if not result:
            ...
            gates[gate_name] = {
                "status": "fail",
                "reason": "test results not collected (pytest timeout or parse error)",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--no-pytest",
        action="store_true",
        help="跳过 pytest 运行（仅输出已知差异和元数据，所有门禁标记为 skipped）",
    )
    parser.add_argument(
        "--repo-root",
        default=None,
    ...
    # 如果 --no-pytest，标记所有门禁为 skipped
    if args.no_pytest:
        for gate_name in report["gates"]:
            report["gates"][gate_name]["status"] = "skipped"
