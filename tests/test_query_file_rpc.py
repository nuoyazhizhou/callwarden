...
class TestClientFailClosed:
    ...
    def test_get_file_symbols_fail_closed_even_in_local_mode(self, monkeypatch):
        """local 模式本地 SQL 路径已随 SRV-006 退役 → 同样 fail-closed。

        stale 依据（A 类，生产侧 server/daemon_client.py:1555 get_file_symbols）：
        旧实现 local 模式经 _get_db() 走本地 SQL（本用例原断言）；现 `_remote_query`
        返回 _NO_REMOTE 后无条件 raise DaemonUnavailableError（_get_db 已退役），
        不再存在本地回退分支。
        """
        from callwarden.server import daemon_client as dc_module
        from callwarden.server.daemon_client import DaemonUnavailableError
