# -*- coding: utf-8 -*-
"""MCP-058: lsp_completion → Rust daemon native 的 HTTP RPC 往返测试。

live-daemon HTTP 往返模式（同 MCP-033~057）：
1. 复用 dev cw-daemon（HTTP manifest 已在 ~/.callwarden）
2. HttpDaemonRpcClient 走 /v1/rpc 调用 lsp_completion
...
def test_lsp_completion_shape(rpc):
    out = _call(rpc, "lsp_completion", {"file_path": "test.py", "line": 3, "character": 5})
    assert isinstance(out, dict)
    # 真相源（db_lsp.lsp_completion）返回 {completions,total,available}，
    # 不含 file_path 回显——旧断言对着未落地契约编写。
    assert out.get("completions") == []
    assert out.get("total") == 0
    assert out.get("available") is False  # daemon 无 LSP server → 降级


def test_lsp_completion_defaults(rpc):
    out = _call(rpc, "lsp_completion", {})
    assert isinstance(out, dict)
    assert out.get("completions") == []
    assert out.get("total") == 0
...
def test_lsp_completion_daemon_unavailable_fail_closed():
    c = HttpDaemonRpcClient(endpoint="http://127.0.0.1:9")
    with pytest.raises(Exception):
        c.call("lsp_completion", {"file_path": "a.py", "line": 1, "character": 1})
...
def test_new_client_instance_stable(rpc):
    ...
    out = _call(c2, "lsp_completion", {"file_path": "a.py", "line": 1, "character": 1})
    assert isinstance(out, dict)
