...
## 0.1 统一状态模型（必须先读 daemon 投影）

复审前必须记录 `next-action` 返回的精确 `task_id`、`lifecycle_status`、`workflow_status`、
`current_role`、`next_role`、`next_action`、`review.state` 和 `blocking_reasons`。其中 raw
`lifecycle_status=review` 只表示生命周期阶段，不表示“没有发生 Reviewer 动作”；治理进度由 daemon
根据 active lease、Verdict Ledger 和派工事实派生，Reviewer 不自行重算。

- `review_pending`：Executor 已 report，等待本 Reviewer。
- 有效 PASS 后为 `adjudication_pending`：raw status 仍可为 `review`，PASS 不等于 `applied`。
- 有效 BLOCKED 后为 `remediation_pending`：daemon 在同一主任务追加 provenance-bound `fix_defect`，
  Executor 领取后为 `remediation_in_progress`。
- `READY/REVIEW` 只表示“可以领取复审”，不是 Reviewer 已领取或已完成；必须以 lease、verdict 和
  daemon 成功响应作为事实。

### Reviewer 发送前 Handoff 检查

`Handoff` 必须第一项写被复核的精确 `task_id`，随后完整写出 `from_role`、`outcome`、`next_role`、
`next_action`、`reason`、`independence_requirement`、`request_id`、`step_id`、`report_request_id`、
`evidence_path`、`evidence_hash` 和五字段 `identity`。PASS 的 `next_action` 只能指向该 task_id 的
独立 Adjudicator 复审；BLOCKED 的 `next_action` 只能指向该 task_id 的 Executor remediation。
不能用“当前任务”、父任务 ID、step/request ID 或自然语言“通过”替代 task_id/provenance，也不能在
当前 Handoff 中安排领取下一个任务。
...
任务绑定：每次处理只针对 next-action 返回的精确 task_id；该 task_id 必须原样出现在 review/verdict/handoff，
不得用 Epic 父任务 ID、step_id 或 request_id 代替，也不得从聊天文本猜测任务。
循环：
1. 先 cw task list --parent T-1787203926824-9f873bfc --workspace-id 1 --json 取出 Epic 子树全部子任务
   ...
   筛出 required_role=reviewer 且 action=review_current_step 的派工投影；保存该响应的精确 task_id、
   workflow_status 和 next_action；无 → IDLE，下一轮再查。
...
5. verdict/handoff 首先附精确 task_id，再附 request_id/step_id/evidence path+hash/identity 全套字段（含
   agent_instance_id）；Reviewer 必须围绕该 task_id 复核并交棒，严禁只在聊天说"通过"。
...
1. **发现。** 先 `cw task list --parent T-1787203926824-9f873bfc --workspace-id 1 --json` 取出 Epic 子树全部
   ...
   `action=review_current_step`（`source.task_status=review`、通常 `workflow_status=review_pending`）的候选。
   后续所有 lease、复审、verdict 和 handoff
   都必须继续使用该候选返回的精确 `task_id`。**两个易错点**：① `task.next_action`
...
5. **结构化交棒。** 不通过 → `reviewer_blocked → executor`（附 findings，`fix_defect` step 由 daemon
   同事务追加，治理状态为 `remediation_pending`）；通过 → `reviewer_pass → adjudicator`（治理状态为
   `adjudication_pending`，raw status 不伪造为 applied）。两种情况均以精确 `task_id` 开头，并附 request_id、step_id、
