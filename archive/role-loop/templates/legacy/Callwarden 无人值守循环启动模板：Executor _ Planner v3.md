...
## 0.1 统一状态模型（必须按 daemon 投影理解）

每次读取 `task.next_action` 或 task governance projection 时，同时记录精确 `task_id`、
`lifecycle_status`、`workflow_status`、`current_role`、`next_role`、`next_action`、`review.state` 和
`blocking_reasons`。`lifecycle_status` 是 raw `tasks.status` 的 apply/close 门禁；`workflow_status` 是
daemon 根据 lease、Verdict Ledger 和派工事实派生的用户可读进度，Executor 不得在客户端重算。

- `queued`：任务已创建但尚未执行；`execution_in_progress`：正在执行或已有 active lease。
- `remediation_pending`：Reviewer/Adjudicator 已退回，等待 Executor 领取修复；领取 `fix_defect` 后为
  `remediation_in_progress`。
- `review_pending`：Executor 已 report，等待 Reviewer。此时 raw status 通常为 `review`。
- Reviewer PASS 后为 `adjudication_pending`，不等于 `applied`；只有 Adjudicator 成功 apply 后才为
  `applied_pending_close`，close 后为 `completed`。

`READY/CLAIM` 只表示“允许领取”，不是“已经领取”；必须看到 `task next` 的成功响应才可记录
`in_progress`。`task.report` 成功后必须以同一 `task_id` 交 Reviewer；不要因为 Reviewer PASS/BLOCKED
没有立即改变 raw status 就认为没有发生事件，后续动作以新的 `next-action` 投影为准。

### Executor 发送前 Handoff 检查

最终响应中的 `Handoff` 第一项必须是 daemon 返回的精确 `task_id`，并完整包含
`from_role`、`outcome`、`next_role`、`next_action`、`reason`、`independence_requirement`、
`request_id`、`step_id`、`report_request_id`、`evidence_path`、`evidence_hash` 和五字段
`identity`（`agent_id`、`agent_instance_id`、`session_id`、`model_id`、`role`）。
`next_action` 只能描述当前 task_id 交给 Reviewer 的动作，不能写“随后领取下一个任务”；
新任务必须等当前任务进入 `review` 并完成交棒后重新执行发现循环。缺任一 provenance 字段时，
不得输出 `executor_ready_for_review`，应报告具体缺口。
...
任务绑定：每次循环只处理 next-action 返回的精确 task_id；task_id 必须贯穿 claim、report、handoff、证据和
commit message，交给 Reviewer 时不得用 Epic 父任务 ID、step_id 或 request_id 代替，也不得从聊天文本猜测。
循环：
...
4. 自测通过 → 证据落地共享目录 → cw task report <id> --step-id <step> \
     ...
     --model-id workbuddy --role executor （自动置 review，workflow_status=review_pending）→ 以该精确 task_id 提交完整 provenance Handoff（executor_ready_for_review）。
...
1. **发现。** 先查询自己名下是否有未完成的 `in_progress` 任务（**续做优先**，补齐到 review）；无则
   ...
   capture instance，如 `4baea3ff12c2ea5c`）。读取 `task_id`（后续所有写操作和交接均使用该精确 ID）、
   `required_role`（必须为 executor）、`action`、
...
5. **证据、交棒与 VCS 卫生。** 步骤全部真实完成且证据可复现时，`cw task report <id> --step-id <step>
   ...
   `next_role=reviewer`、`next_action=独立复审该 task_id`、`independence_requirement=required`），附 evidence
