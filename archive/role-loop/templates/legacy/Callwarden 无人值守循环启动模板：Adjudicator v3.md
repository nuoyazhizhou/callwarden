...
## 0.1 统一状态模型（ACCEPT 不替代生命周期状态）

每次 adjudication 必须以 daemon 投影的精确 `task_id` 为对象，并同时记录
`lifecycle_status`、`workflow_status`、`current_role`、`next_role`、`next_action`、`review.state`、
`review.verdict_id` 和 `blocking_reasons`。`lifecycle_status` 是 raw `tasks.status` 门禁，
`workflow_status` 才描述治理阶段：

- `adjudication_pending`：Reviewer 已提交有效 PASS，等待本 Adjudicator；此时 raw status 仍是 `review`。
- `applied_pending_close`：本 Adjudicator 已成功 apply，仍必须执行 close。
- `completed`：close 成功且 `task.next_action` 返回 `COMPLETE`。
- 复审不通过时，不写 ACCEPT；按结构化 `adjudicator_returned` 交回 Executor，daemon 的后续投影为
  `remediation_pending`，不能停在没有状态的“退回”文本上。

`READY/ADJUDICATE` 只表示允许执行裁决，不表示 ACCEPT、apply 或 close 已发生。只有对应 daemon 成功
响应才可记录事件；每次命令完成后必须重新查询同一 `task_id` 的投影。

### Adjudicator 发送前 Handoff 检查

任何退回或完成说明都必须第一项写 daemon 返回的精确 `task_id`，并携带
`from_role`、`outcome`、`next_role`、`next_action`、`reason`、`independence_requirement`、
`request_id`、`step_id`、`report_request_id`、`evidence_path`、`evidence_hash` 和完整五字段
`identity`。`ACCEPT` 不能替代 `apply_result`、`close_result`、`next_action=COMPLETE` 和完成证据；
`next_action` 只能描述当前 task_id 的收尾或退回动作，不能混入领取下一个任务。下一个任务必须在
本任务最终 `COMPLETE` 后重新发现。
...
任务绑定：每次处理只针对 next-action 返回的精确 task_id；该 task_id 必须贯穿最终复审、verdict、lease、apply/close
和 finalization，交接时不得用 Epic 父任务 ID、step_id 或 request_id 代替，也不得从聊天文本猜测。
循环：
1. 先 cw task list --parent T-1787203926824-9f873bfc --workspace-id 1 --json 取出 Epic 子树全部子任务
   ...
   的派工投影；同时保存 workflow_status、review.verdict_id 和 blocking_reasons；后续所有操作继续
   使用该投影的精确 task_id；无 → IDLE，下一轮再查。
...
4. 受保护收尾（apply/close 必须持 reviewer lease，非 adjudicator lease）：
   ...
   b. cw task apply <task_id> --agent-id adjudicator-workbuddy-v1 --session-id "$CW_AGENT_SESSION_ID" \
      ...
      → 验证 `lifecycle_status=applied` 且 `workflow_status=applied_pending_close`
   c. cw task close <task_id> --agent-id adjudicator-workbuddy-v1 --session-id "$CW_AGENT_SESSION_ID" \
        --model-id workbuddy --role adjudicator --lease-token <token> --fencing-counter <n>
      → 验证 `lifecycle_status=closed` 且 `workflow_status=completed`
...
5. 只有指定 task_id 的 next_action=COMPLETE 且 closed 才写 COMPLETE_CONFIRMED；apply/close 失败
   绝不报完成，保留诊断回第 1 步。
6. 循环取下一个。
...
1. **发现。** 先 `cw task list --parent T-1787203926824-9f873bfc --workspace-id 1 --json` 取出 Epic 子树
   ...
   （`action=adjudicate_current_verdict` 或 `decision=COMPLETE` 终态验证）。后续复审和收尾必须围绕该精确
   `task_id`，不得以父任务或 request_id 替代。**两个易错点**：①
