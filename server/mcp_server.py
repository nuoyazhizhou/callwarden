...
def create_mcp_server():
    ...
    #
    # 设计依据：docs/design/multi-llm-contract-driven-collaboration-design.md §13.5
    #   - Role_View 获取 / Evidence 查询 / Freshness_Status 查询 / gate decision 查询
    #     均为 MCP 只读工具，不触发任何写操作（含 workspace 激活一类隐式 UPDATE）
    #   - 只读返回的 Freshness_Status 是查询时刻派生值，不构成 gate 结论
    #   - D0 未完成前 P1–P4 标记 planned/unavailable（tasks.md 3.23）
    #
    # 实现约束：
    #   1. 不调用 get_db(workspace) 以避免 workspace 激活写操作；db_path 仅通过
    #      _get_db_path_for_daemon() 读取当前 workspace_root（不切换）
    #   2. 通过 call_with_autostart 走 daemon：只读方法天然归类为 READ_ONLY
    #      （degraded_mode.py），允许 direct_read 降级，不 fail closed
    #   3. P1 schema 未创建时（当前 D0 阶段），daemon 端未注册对应 RPC 方法，
    #      返回结构化 "planned/unavailable" 响应——这是能力未启用的正常响应，
    #      不是失败路径
    #   4. 真正的异常路径（daemon RPC 错误）返回 Structured_Reason：
    #      稳定错误码 + i18n key（通过 t(key, default=...) 解析，后续由 3.23
    #      文档同步任务正式注册到 i18n catalog）
    # ----------------------------------------------------------------

    def _collab_planned_response(tool_name: str, stage: str = "P1") -> dict:
        """构造"阶段未启用"的结构化响应（非错误，能力未交付的正常响应）。

        Args:
            tool_name: 工具名称（用于上下文）
            stage: 阶段标识（P1/P2/P3/P4）

        Returns:
            结构化响应 dict，包含 status / stage / tool / message_key / message
        """
        message_key = "info.collab_stage_planned"
        msg = t(
            message_key,
            default=f"能力未启用：{tool_name} 依赖 {stage} 阶段，当前 {stage} 尚未启用（planned/unavailable）。",
            tool=tool_name,
            stage=stage,
        )
        return {
    ...
    def _collab_rpc_call(tool_name: str, method: str,
        ...
        2. daemon 端未注册方法（P1 未启用）→ 返回 planned/unavailable
        3. daemon 不可用 + 降级 direct_read → P1 schema 不存在 → 返回 planned/unavailable
        4. 其他异常 → 返回 Structured_Reason
        ...
        Returns:
            查询结果 dict 或 planned/unavailable 响应或错误响应
        """
        try:
            client = _get_daemon_client()
            response = client.call_with_autostart(method, params)
        except Exception as exc:
            # daemon 不可用且降级失败：P1 schema 不存在，视为 planned/unavailable
            # （daemon 端未注册这些方法，P1 阶段才会产品化）
            return _collab_planned_response(tool_name)

        # 降级路径：只读方法允许 direct_read，但 P1 schema 不存在
        if response.get("degraded"):
            return _collab_planned_response(tool_name)

        result = response.get("result")
        if isinstance(result, dict) and result.get("error"):
            # daemon 返回了错误（如 method not found）
            err = result["error"]
            err_msg = err.get("message", str(err)) if isinstance(err, dict) else str(err)
            # method not found 表示 P1 未启用，返回 planned/unavailable
