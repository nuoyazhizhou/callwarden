"""daemon_registry.py — Enterprise daemon workspace registry 数据库层

db/ 退休 phase-3：原 callwarden/db/db_daemon.py 整体搬迁至本模块。
提供 workspace 注册、查询、状态管理 + container mount mapping CRUD。
语义与原 db_daemon 逐行一致；db_daemon.py 改为重导出壳供过渡期使用。
"""
...
def init_daemon_schema(conn: sqlite3.Connection) -> None:
