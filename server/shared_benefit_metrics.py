...
# L7（2026-07-20 批次4）：get_process_rss_mb() 已迁移到
# callwarden.server.metrics.get_memory_info()，由 daemon metrics 采样线程
# 定期调用刷新 G13 MetricsCollector gauge，避免多处重复实现 RSS 采集。
# Windows Psapi / Linux /proc/self/status fallback 统一在 metrics.py 维护。
