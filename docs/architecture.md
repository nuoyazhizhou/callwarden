...
│  子命令 + --flag 双风格  │     │   195 个 @mcp.tool() 工具    │
│  145+ 命令               │     │   stdio / SSE 传输           │
└────────────┬─────────────┘     └──────────────┬───────────────┘
             │                                  │
             └──────────────┬───────────────────┘
...
Call Warden 把 145+ 个 CLI 命令和 195 个 MCP 工具按功能聚合为 12 个主分类，CLI 与 MCP 共用同一套分类体系。
...
| 工作项 | 状态 | 说明 |
|--------|------|------|
| MCP 工具命名审计 | ✅ | 195 个 `@mcp.tool()` 全量审计，结论：无严重不一致（详见 `.mcp_audit.md` §3） |
...
- [MCP 工具参考](mcp_tools.md)：195 个工具详情
