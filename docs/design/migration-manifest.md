...
SCHEMA_VERSION = 59
RUST_SCHEMA_VERSION = 59

- `SCHEMA_VERSION`（Python 真相源，`db/schema.py`）：v59 = task.supersede 治理权威 schema（T-1787277487109-758e56d0，P0-H）——将 `task_supersede_relations` / `task_supersede_events` 从 Rust 启动期临时 DDL（`ensure_supersede_schema`）纳入 checksummed canonical schema，并补齐 workspace/provenance 列（workspace_id、supersedence_id、reason_code、actor agent/session/model/role、request_id、lease_id/fencing_counter、evidence path/hash、authoritative_timestamp）与 workspace/supersedence 索引；既有库经 `db_base._migrate_v58_to_v59` ALTER ADD COLUMN 无损补齐（历史行不动），Rust 侧同步 `RUST_SCHEMA_VERSION=59` 并在 `TaskCollabStore` 打开权威库时以列级校验替代旧启动期 DDL。v58 = 补 v53-v57 表/列定义漂移（T-1787203926824-9f873bfc）。commit 030fd8c 在 v57 已发布后向 SCHEMA_SQL 补入 11 张 v53-v57 表（canonicalization_rule_sets / canonicalization_rule_revocations、task_operation_ledger、workspace_authority_captures、task_workspace_bindings、task_loop_capability_promotion_events、role_contract_lineages / role_contract_revisions、task_step_role_contract_bindings、verdict_normalization_rules / verdict_normalization_rule_revocations）与既有表的 20 个新列（task_contract_revisions ×2、task_verdict_events ×9、task_gate_decisions ×9），但未递增版本号，导致 v57 库 checksum 失配且缺表 → Rust storage fail-closed。v58 强制走迁移路径：CREATE TABLE IF NOT EXISTS 幂等补建缺失表 + ALTER TABLE ADD COLUMN 补列（lossless），随后重写 stored checksum。v57 = Verdict/Gate schema 与 legacy fail-closed（cw-role-handoff-task-loop.md §4.2/§8.1，1E）——新增 `verdict_normalization_rules` / `verdict_normalization_rule_revocations` 两表 + `('verdict-normalization/v1')` 初始 rule row；`task_contract_revisions` 增加 `normalization_version` / `normalization_rules_hash` 绑定；`task_verdict_events` / `task_gate_decisions` 各增加 Role Contract 绑定 provenance 列（`step_id`、`role_contract_lineage_id`、`role_contract_revision_id`、`role_contract_revision`、`role_contract_hash`、`canonicalization_version` / `canonicalization_rules_hash`）与所用 normalization version/hash；历史 verdict/gate/contract 无绑定只读显示 `UNVERIFIED`，不回填改写。v56 = step→Role Contract binding（§8.1.4，1C）。v55 = Role Contract lineage/c14n（§8.1.2，1B）。v54 = task_loop 公共能力 promotion 权威账本（§8.1.5，1D3B）。v53 = Task workspace authority binding（§8.1.1，1A）。v52 = Task-domain operation store（§8.1.3/§8.1.4，1D1）。v51 = Workspace runtime policy（self_bootstrap / standard）。v50 = Agent Identity + Role Contract。v49 = 复审 P0-1 修复 + v48 guardrail_findings workspace_id 归属 + v47 task_events / agent_registrations + v46 P4 assignment/lease 三表。
- `RUST_SCHEMA_VERSION`（Rust 官方迁移常量，`rust_ext/src/sqlite_query.rs`）：**v59 已同步**。daemon TaskCollabStore 打开权威任务库时执行 `migrate_connection` 正式迁移并校验实际 schema version，与 Python 迁移幂等等价。v59 将 supersede 双表纳入 canonical schema（`include_str!` 内嵌 schema.py），并在 `TaskCollabStore::new/from_connection` 以 `validate_supersede_schema` 列级 fail-closed 校验替代旧启动期 DDL（`ensure_supersede_schema` 已移除）。v58 修复 v57 发布后 schema.py 补表/补列未 bump 版本号的漂移：`execute_existing_schema`（列级 compat 清单已含 v57 的 20 个新列）ALTER 补齐既有表 + CREATE TABLE IF NOT EXISTS 补建 11 张缺失表 + 幂等播种；`storage.rs` 的 `missing_compat_columns` 同步增加列级校验，迁移事务先 `apply_task_contract_v57_compat` 补列再执行 canonical SCHEMA_SQL。镜像常量 `abi_contract.rs:SCHEMA_VERSION` 已同步为 59。
