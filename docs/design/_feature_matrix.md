...
| H14 | 跨平台打包发布（MSI/PKG/DEB） | CP | 🟡 部分完成（2026-07-25 P0-P2） | 发布 spec 已改为单一共享 `_internal`：Windows/macOS 只冻结 `cw`，Linux 冻结 `cw/cw-client/cw-agent`；Rust 扩展和 Call Warden 核心模块缺失会 fail closed；冻结包必须通过 `cw server --check-imports`。Windows x64 真实本地构建通过：131.41 MB 解压、41.84 MB zip、1939 模块、无禁止云 SDK；发布/enterprise CI 已增加 JSON 包体清单和平台体积门禁。**仍未完成**：MSI/PKG/DEB 的干净 runner 安装 E2E、macOS 签名公证、Windows Authenticode、Linux 双 UID/systemd 安装验收，因此不能标 ✅。 |
...
| N6 | macOS arm64 pkg + notarization（原 universal2，2026-07-22 降级） | CP | 🟡 部分完成（2026-07-25） | `build_pkg.sh` 只发布 macOS 支持的 `cw` local 角色并复用唯一 `_internal`，其他角色 fail closed；v0.3.2 macOS arm64 PyInstaller smoke、门禁和 29.20 MB tar.gz Release 资产已通过。**剩余**：pkg、codesign、notarization、stapling、Gatekeeper E2E 尚未验收。 |
| N7 | Linux deb 5 子包 + tar.zst（RPM 不在发布范围） | CP | 🟡 部分完成（2026-07-25） | Linux PyInstaller 构建目录内 `cw`/`cw-client`/`cw-agent` 共享唯一 `_internal`；v0.3.2 三入口 smoke、门禁和 76.96 MB tar.gz Release 资产已通过，daemon 仍为 Rust 二进制。**剩余**：deb 五角色安装、dpkg、systemd/双 UID E2E 尚未验收。 |
| N8 | Release CI enterprise-release.yml（批次5 文档对齐） | CP | 🟡 部分完成（2026-07-25） | v0.3.2 `PyInstaller Build & Release` 三平台 build、入口 smoke、制品门禁、上传和 GitHub Release 全部成功；资产为 Linux 76.96 MB、macOS 29.20 MB、Windows 37.15 MB。冻结构建使用固定版本白名单，不安装 wheel `[all]`/torch。**剩余**：独立 `enterprise-release.yml` 仍在 GitHub workflow 门禁阶段失败，MSI/pkg/deb 企业安装链尚未全绿。 |
