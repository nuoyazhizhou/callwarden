...
SYSTEM_SEMGREP_RULES_DIR = os.path.join(
    SYSTEM_CACHE_DIR, "semgrep_rules") if SYSTEM_CACHE_DIR else ""
...
def scan_subprojects(root_dir: str, max_depth: int = 5,
    ...
    for root, dirs, files in os.walk(root_dir, onerror=_scan_onerror):
        ...
        # 跳过第三方/VCS/构建目录
        dirs[:] = [
            d for d in dirs if d not in _SUBPROJECT_SKIP_DIRS and not d.startswith(".")]
...
def auto_generate_ignore(project_root: str, dry_run: bool = True) -> Dict:
    ...
    for root, dirs, files in os.walk(project_root, onerror=lambda e: None):
        ...
        dirs[:] = [d for d in dirs if not d.startswith(
            ".") and d not in _KNOWN_THIRD_PARTY_DIRS]
    ...
    if not dry_run and new_patterns:
        ...
        # 生成追加内容
        append_lines = [
            "", "# === auto-generated by cw workspace generate-ignore ==="]
        append_lines.append(f"# Total: {len(new_patterns)} new patterns")
        append_lines.append(
            "# (default baseline like .git/ node_modules/ build/ are built-in, not listed here)")
        append_lines.append("")
...
# ────────────────────────────────────────────────────────────────────────────
# 原 db/rust_parser_facade.py 内的 ParseMode，db/ 退休 phase-3 下沉至 config。
# 本类仅提供模式判断与环境校验（纯配置，不依赖 db 包），cw.py 的
# _enforce_frozen_parse_mode() 与 RustParserFacade 共用。
