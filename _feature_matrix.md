...
| G8 | Watcher Generation 状态机 | WG/EW | ✅ 已闭合（2026-07-22 P0-2 三子问题全部修复） | session/generation CAS 和 CAS publish 存在；canonical_bytes 协议 P0-2 已修复。**P0-1 整改（2026-07-21）**：Python `daemon_handle_refresh` CAS committed 后调用 `merge_cas_to_codegraph` + `upsert_manifest`。**P0-2 整改（2026-07-22）Python 路径**（commit dc33c1f）：(1) step 4 (committed_generation) 移到 step 5 (merge/manifest) 之后；(2) step 2 stale_seq 逻辑放宽；(3) `merge_cas_to_codegraph` 补 symbol_contents UPSERT + callee_id 入边清理。8 测试通过。**P0-2 子问题1 修复（2026-07-22，commit 4fa30ef）**：Rust daemon `publish_snapshot` 路径新增 `cas_merge::merge_cas_to_codegraph`（[rust_ext/src/daemon/cas_merge.rs](file:///c:/git_work/callwarden/rust_ext/src/daemon/cas_merge.rs)），移植 Python merge 逻辑，7 个单元测试通过，replicator 43 个测试无回归。**P0-2 子问题3 修复（2026-07-22，commit 349ab9a）**：Rust GraphStore SQL 添加 `workspace_id` 过滤条件，避免 snapshot 混入其他 workspace 数据。 |
...
| G11 | Replicator（CAS → Manifest → Snapshot） | DS/EW | ✅ 已闭合（2026-07-22 P0-2 三子问题全部修复） | `DaemonConfig.codegraph_db_path_template` 默认空字符串问题通过 dispatch 层从 `res["codegraph_db_path"]` 显式传入绕过。**P0-1 整改（2026-07-21）**：Python `daemon_handle_refresh` CAS committed 后新增 step 5——调用 `merge_cas_to_codegraph` + `upsert_manifest`。**P0-2 整改（2026-07-22）Python 路径**（commit dc33c1f）：(1) step 4 后置到 step 5 之后，避免非原子提交；(2) step 2 stale_seq 逻辑放宽，允许同 seq 未 committed 时重试；(3) `merge_cas_to_codegraph` 补 symbol_contents UPSERT 修复 `symbols.symbol_hash` 外键断链；(4) 补 callee_id 入边清理避免悬空引用。8 测试通过。**P0-2 子问题1 修复（2026-07-22，commit 4fa30ef）**：Rust daemon `publish_snapshot` 路径新增 `cas_merge::merge_cas_to_codegraph`（[rust_ext/src/daemon/cas_merge.rs](file:///c:/git_work/callwarden/rust_ext/src/daemon/cas_merge.rs)），移植 Python merge 逻辑，7 个单元测试通过，replicator 43 个测试无回归。**P0-2 子问题3 修复（2026-07-22，commit 349ab9a）**：Rust GraphStore SQL 添加 `workspace_id` 过滤条件。 |
...
| J8 | Daemon UDS 协议闭合 | ✅ 已闭合（2026-07-22 P0-2 三子问题全部修复） | Rust 端 SnapshotDaemonState 已实现 36 个 handle_xxx；Python daemon_client 三个高级查询方法补齐 RPC 路径；CLI 子命令齐全。**复审回退（2026-07-21）→部分修复**：(1) Rust `memfd.rs` 无 `F_GET_SEALS` 也无 owner UID 校验→**P1-3 已修复**（六重校验）；(2) `daemon_handle_refresh` 只更新 CAS/generation，未写 `workspace_manifests` 也未对 symbols/calls 执行 delta apply→**P0-1 Python 路径已修复**（新增 `db_cas_merge.py` + `upsert_manifest`）；(3) `codegraph_db_path_template` 默认空→**P0-1 已修复**（dispatch 层显式传入）；(4) workspace_id 级 ACL 缺口→**Python 端已修复**（5 个 handler 接入 `_owned_workspace_by_id`）；(5) dispatch 层 `int(workspace_id)` bug→**已修复**。**P0-2 整改（2026-07-22）**：(1) Python 路径 step 4 后置 + step 2 stale_seq 放宽 + symbol_contents/callee_id 修复（commit dc33c1f）；(2) Rust daemon 路径新增 `cas_merge::merge_cas_to_codegraph`（commit 4fa30ef），7 测试通过，replicator 43 测试无回归；(3) Rust GraphStore SQL 添加 `workspace_id` 过滤条件（commit 349ab9a），避免 snapshot 混入其他 workspace 数据。 |
...
| K4 | daemon dispatch 未接入 | ✅ 已闭合（2026-07-22 P0-2 三子问题全部修复） | **已修复（2026-07-21 P0-1 整改）**：`daemon_server.py` `workspace.file.refresh` 调用 `daemon_handle_refresh` + staging + replicate 存在；dispatch 层 `int(workspace_id)` bug 修复为 `int(workspace["workspace_id"])`；`daemon_handle_refresh` CAS committed 后新增 step 5 调用 `merge_cas_to_codegraph` + `upsert_manifest`；`codegraph_db_path_template` 默认空问题通过 `res["codegraph_db_path"]` 显式传入绕过。**P0-2 整改（2026-07-22）**：(1) Python 路径 step 4 后置 + step 2 stale_seq 放宽 + symbol_contents/callee_id 修复（commit dc33c1f），8 测试通过；(2) Rust daemon 路径新增 `cas_merge::merge_cas_to_codegraph`（commit 4fa30ef），7 测试通过，replicator 43 测试无回归；(3) Rust GraphStore SQL 添加 `workspace_id` 过滤条件（commit 349ab9a）。企业部署 save-to-query 数据链已闭合。 |
