...
| G9 | Per-UID systemd --user agent | DS/WG | 🟡 复审回退（2026-07-21） | AgentSession/Watcher/systemd unit 存在；hex/b64 协议已修复（批次3）；包入口名不一致已修复（批次14）。**复审回退（2026-07-21）**：`cli/main.py:_agent_start` 先调用 `workspace.connect` 后才注册 workspace，全新 agent 无法连接；客户端 workspace_instance_id 算法（项目路径 hash）与 daemon 注册 ID 算法（owner/root/remote/commit hash）不一致。agent fresh start 协议不可用 |
| G10 | memfd 密封协议（大文件传输） | DI | 🟡 复审回退（2026-07-21） | Python 路径 `agent_protocol.py:307-313` 使用 `create_sealed_memfd`；`daemon_server.py:798-802` 通过 `is_memfd` + `validate_memfd_fd` 四重校验。**复审回退（2026-07-21）**：Rust 端 `rust_ext/src/daemon/memfd.rs` 当前检查常规文件类型、大小、容量和可选 hash，**没有 `F_GET_SEALS`，也没有校验 FD owner 与 peer UID**。Python 和 Rust 两条实现安全等级不一致 |
| G11 | Replicator（CAS → Manifest → Snapshot） | DS/EW | ❌ 复审回退（2026-07-21） | `DaemonConfig.codegraph_db_path_template` 默认空字符串；release/systemd/deployment 未设置 `CW_DAEMON_CODEGRAPH_DB_TEMPLATE`。即使手工设置，replicator 只 reload 外部 DB 不先 delta apply；`daemon_handle_refresh` 只更新 CAS/generation，**不写 `workspace_manifests`，不应用 delta 到 symbols/calls**。save-to-query 链路完全断裂 |
...
| H14 | 跨平台打包发布（MSI/PKG/DEB） | CP | ❌ 复审回退（2026-07-21） | **复审报告 §3 P0-3**：真实运行 `python release/build.py --wheel` 失败（`--config-setting --build-option=--plat-name=win_amd64` 参数错误，exit 非零）。当前唯一 wheel `release/dist/callwarden-0.3.0-py3-none-any.whl` 不含 `callwarden_core` Rust 扩展。`release/build.py` 不生成 `dist/linux/cw`、`cw-client`、`cw-agent`、`cw-daemon`；WiX 引用 `cw.exe`/`cw-client.exe`/`runtime/python.exe` 但 workflow 不生成这些输入；macOS 脚本 `APPLE_*` 与 `CW_APPLE_*` 环境变量不一致，签名/公证被跳过。主 CLI 不支持 `cw --version`（exit 2），Release Gate 3 首个黑盒命令必失败。N5/N6/N7/N8 全部回退为 ❌。证据：复审报告 `docs/design/feature-matrix-code-reaudit-2026-07-21.md` §3 P0-3 |
...
| J8 | Daemon UDS 协议闭合 | ❌ 复审回退（2026-07-21） | Rust 端 SnapshotDaemonState 已实现 36 个 handle_xxx；Python daemon_client 三个高级查询方法补齐 RPC 路径；CLI 子命令齐全。**复审回退（2026-07-21）**：协议链路声称"闭合"与源码不符——(1) Rust `memfd.rs` 无 `F_GET_SEALS` 也无 owner UID 校验（见 G10）；(2) `daemon_handle_refresh` 只更新 CAS/generation，未写 `workspace_manifests` 也未对 symbols/calls 执行 delta apply（见 G8/G11）；(3) `codegraph_db_path_template` 默认空，release/systemd/deployment 未设 `CW_DAEMON_CODEGRAPH_DB_TEMPLATE`（见 G11）。端到端协议链未真正闭合 |
...
| K4 | daemon dispatch 未接入 | 中 | ❌ 复审回退（2026-07-21）：`daemon_server.py` `workspace.file.refresh` 调用 `daemon_handle_refresh` + staging + replicate 存在。**复审回退（2026-07-21）**：协议链路声称"完整闭合"与源码不符——`codegraph_db_path_template` 默认空导致 `SnapshotCachePublisher` 不注入；replicator 只 reload 外部 DB 不先 delta apply；`daemon_handle_refresh` 不写 `workspace_manifests`，不对 symbols/calls 执行 delta apply。数据发布链完全断裂（见 G8/G11/J8） |
...
| M4 | delta.rs | SymbolDeltaKind（Added/Removed/Changed）+ lang_from_extension（13 语言） | 🟡 复审回退（2026-07-21） | daemon workspace.rs refresh committed 路径调用 DeltaComputer::compute_parse_delta 填充 StagingEntry.parse_delta JSON 摘要。**复审回退（2026-07-21）**：源码明确使用 `store=None` 退化模式，结果只写 staging JSON，未应用到 GraphSnapshot |
| M5 | frontier.rs | AffectedFrontier（directly_affected + upstream/downstream direct/transitive） | 🟡 复审回退（2026-07-21） | daemon workspace.rs refresh committed 路径调用 FrontierComputer::compute_frontier_with_budget 填充 StagingEntry.frontier JSON 摘要。**复审回退（2026-07-21）**：frontier 没有 upstream/downstream，结果只写 staging JSON，未应用到 GraphSnapshot |
| M6 | metrics.rs | DepthChange + CycleChangeKind（Added/Removed） | 🟡 复审回退（2026-07-21） | daemon workspace.rs refresh committed 路径调用 MetricsComputer::compute_local_update 填充 StagingEntry.metrics_update JSON 摘要。**复审回退（2026-07-21）**：metrics 没有旧图对比，结果只写 staging JSON，未应用到 GraphSnapshot |
...
| N3 | release/build.py 构建管道（批次5 文档对齐） | CP | ❌ 复审回退（2026-07-21） | **复审报告 §3 P0-3**：真实运行 `python release/build.py --wheel` 失败（`--config-setting --build-option=--plat-name=win_amd64` 参数错误，exit 非零）。当前唯一 wheel 是 `release/dist/callwarden-0.3.0-py3-none-any.whl` 不含 `callwarden_core`。`release/build.py` 不生成 `dist/linux/cw`、`cw-client`、`cw-agent`、`cw-daemon`。CI 先在 `rust_ext/target` 构建，但 `--wheel` 要求根目录已存在 `.pyd/.so`；干净 runner 不会自动复制，Gate 2 会更早失败。证据：复审报告 `docs/design/feature-matrix-code-reaudit-2026-07-21.md` §3 P0-3 |
...
| N7 | Linux deb 5 子包 + tar.zst（RPM 不在发布范围） | CP | ❌ 复审回退（2026-07-21） | **复审报告 §3 P0-3**：批次14 fail-fast 修复只是把 `\|\| true` 改成 `exit 1`，但 Linux 包脚本仍依赖 `cw-daemon` 二进制（连字符），而 Cargo 只声明 `cw_daemon`（下划线），systemd unit 又执行 `cw_daemon`，**三处命名不一致**。即使脚本运行，因 `cw-daemon` 二进制不存在仍会立即 fail-fast exit 1。Linux offline bundle 调用参数顺序错误且带 `\|\| true`；workflow 上传的 `manifest.json` 路径与脚本产出不一致。证据：复审报告 `docs/design/feature-matrix-code-reaudit-2026-07-21.md` §3 P0-3 |
| N8 | Release CI enterprise-release.yml（批次5 文档对齐） | CP | ❌ 复审回退（2026-07-21） | **复审报告 §3 P0-3**：version key 修正可保留，但 workflow 未实际运行过。WiX 引用 `cw.exe`/`cw-client.exe`/`runtime/python.exe` 但 workflow 不生成这些输入；macOS 脚本 `APPLE_*` 与 `CW_APPLE_*` 环境变量不一致，签名/公证被跳过；Linux offline bundle 调用参数顺序错误且带 `\|\| true`；workflow 上传的 `manifest.json` 路径与脚本产出不一致。Release CI 链不可执行。证据：复审报告 `docs/design/feature-matrix-code-reaudit-2026-07-21.md` §3 P0-3 |
