...
1. **高优先级（性能/稳定性）**：L9（Rust ParseResultPool）（L6 部分实现：versions 写入后释放 symbols + 调用图从 DB 读取符号索引；parse 阶段流式回传未实现；F12 快照 dump 已实现；F13 索引精简已实施；L7 RSS 监控已修复；L8 增量调用图已实现；K1-K4/K6 daemon 闭合已全部修复）
