...
| D7 | 跨仓库依赖检测 + 共享符号 + 影响传播 | IS/CL | ✅ 已修复（2026-07-22 P1-2） | db_cross_repo.py / db/schema.py / db/db_base.py（**P1-2 复审整改 2026-07-22**：1) schema v41 — `cross_repo_deps` 加 `idx_cross_repo_unique` 五元组 UNIQUE 索引（源仓库/目标仓库/源 hash/目标 hash/依赖类型），配合 `INSERT OR IGNORE` 实现幂等；2) `_migrate_v40_to_v41` 函数含既有重复记录去重逻辑（保留最大 id 即最新记录）；3) `detect_cross_repo_deps` 重写：`Dict[name, Tuple]` → `Dict[name, List[Tuple]]` + FQN 反向索引，三级优先级匹配（FQN 全匹配 0.95 > FQN 后缀匹配 0.85 > 短名匹配 0.7），同一轮扫描内 `recorded_pairs` set 去重（key 含 target_workspace_id 防跨仓库误去重），删除 `break` 允许多仓库匹配。**复审整改-v2 闭合（2026-07-22 P1-2）**：(a) `cross_repo_impact()` 方向修正——查 `WHERE target_symbol_hash = ?` 找 source_workspace（谁依赖了我），而非原 `WHERE source_symbol_hash = ?` 找 target_workspace（我依赖了谁）；(b) 同轮去重 key 从 `(source_hash, target_hash)` 改为 `(source_hash, target_hash, t_id)` 防跨 workspace 误去重；(c) 移除 `import_path.endswith(cand_qn.split(".")[-1])` 恒真条件（对按同名筛出的候选恒真），只保留 `import_path.endswith(cand_qn)` 完整 FQN 后缀匹配，短名匹配 0.7 分支现在可达。17 测试全部通过） |
...
| # | 功能点 | 来源 | 状态 | 备注 |
|---|--------|------|------|------|
| G1 | 三层存储（Global CAS / Toolchain / Thin Workspace） | EA/DS | 🟡 部分修复（2026-07-22 P0-1 Python ACL + Rust daemon 未闭合） | CAS/toolchain/workspace 三层存在。**P0-1 整改**：ADMIN_ONLY_METHODS + is_admin 顶层 fail-closed。**P0-2 整改（2026-07-21）**：Python 端 5 个 handler 接入 `_owned_workspace_by_id` workspace owner ACL。**复审整改-v2 回退（2026-07-22）**：Rust system daemon 的 5 个 handler（`handle_toolchain_resolve` / `handle_build_context_list` / `handle_resolved_edges_store/get/count`，[snapshot_state.rs:960-1149](file:///c:/git_work/callwarden/rust_ext/src/daemon/snapshot_state.rs#L960)）仍把参数命名为 `_peer` 并忽略，直接使用客户端传入的 `workspace_id`。Python ACL 修复不能代表 Rust daemon 闭合。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md#p0-1-rust-system-daemon-仍可跨-uid-访问-workspace-数据) |
...
| G3 | UDS + SO_PEERCRED 认证 | DS/DI | 🟡 部分修复（2026-07-22 P0-1 Python ACL + Rust daemon 未闭合） | SO_PEERCRED 认证 + workspace owner 过滤存在。**P0-1 整改**：ADMIN_ONLY_METHODS + is_admin 顶层 fail-closed。**P0-2 整改（2026-07-21）**：Python 端 5 个 handler 接入 `_owned_workspace_by_id` workspace owner ACL；Rust `handle_snapshot_list_workspaces` 按 peer_uid 过滤。**复审整改-v2 回退（2026-07-22）**：Rust system daemon 的 5 个 workspace-id handler 仍忽略 `_peer`（[snapshot_state.rs:960-1149](file:///c:/git_work/callwarden/rust_ext/src/daemon/snapshot_state.rs#L960)）。Python ACL 修复不能代表 Rust daemon 闭合。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
| G4 | Workspace Registry + Container Mount Mapping | DS/RP | 🟡 部分修复（2026-07-22 P0-1 Python ACL + Rust daemon 未闭合） | registry 和 mount CRUD 存在。**P0-1 整改**：mount.register/delete 加入 ADMIN_ONLY_METHODS。**P0-2 整改（2026-07-21）**：`mount.list` 加入 ADMIN_ONLY_METHODS。**复审整改-v2 回退（2026-07-22）**：Rust system daemon 的 workspace_id ACL 仍未闭合（同 G1/G3）。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| G8 | Watcher Generation 状态机 | WG/EW | ❌ 未闭合（2026-07-22 复审整改-v2 回退） | session/generation CAS 和 CAS publish 存在；canonical_bytes 协议 P0-2 已修复。**P0-1 整改（2026-07-21）**：Python `daemon_handle_refresh` CAS committed 后调用 `merge_cas_to_codegraph` + `upsert_manifest`。**P0-2 整改（2026-07-22）Python 路径**：(1) step 4 (committed_generation) 移到 step 5 (merge/manifest) 之后；(2) step 2 stale_seq 逻辑放宽；(3) `merge_cas_to_codegraph` 补 symbol_contents UPSERT + callee_id 入边清理。8 测试通过。**复审整改-v2 回退（2026-07-22）**：(1) Rust daemon 路径降级为 CAS-only（`publish_snapshot` 仅重载 SQLite 不做 CAS merge，[replicator.rs:740-769](file:///c:/git_work/callwarden/rust_ext/src/daemon/replicator.rs#L740)），systemd 启动的是 Rust daemon 不是 Python daemon，企业部署 save-to-query 数据链实际未闭合；(2) Python 路径 `merge_cas_to_codegraph` 跨库不原子（单独提交 CodeGraph DB，再提交另一数据库里的 manifest）；(3) Python 默认对所有 workspace 使用同一个用户级 `callwarden.db`，Rust GraphStore 装载 symbols 的 SQL 没有 `workspace_id` 条件，snapshot 内部会混入其他 workspace/UID 数据。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-2/P0-3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| G11 | Replicator（CAS → Manifest → Snapshot） | DS/EW | ❌ 未闭合（2026-07-22 复审整改-v2 回退） | `DaemonConfig.codegraph_db_path_template` 默认空字符串问题通过 dispatch 层从 `res["codegraph_db_path"]` 显式传入绕过。**P0-1 整改（2026-07-21）**：Python `daemon_handle_refresh` CAS committed 后新增 step 5——调用 `merge_cas_to_codegraph` + `upsert_manifest`。**P0-2 整改（2026-07-22）Python 路径**：(1) step 4 后置到 step 5 之后，避免非原子提交；(2) step 2 stale_seq 逻辑放宽，允许同 seq 未 committed 时重试；(3) `merge_cas_to_codegraph` 补 symbol_contents UPSERT 修复 `symbols.symbol_hash` 外键断链；(4) 补 callee_id 入边清理避免悬空引用。8 测试通过。**P0-2 整改（2026-07-22）Rust daemon 路径降级为 CAS-only**：`publish_snapshot` 仅重载 SQLite 不做 CAS merge，已在 `replicator.rs:740-769` + `daemon_handle_refresh` 函数注释 + systemd unit 文件三处加文档说明，企业部署需改用 Python daemon。**未修复（跨库不原子）**：`merge_cas_to_codegraph` 单独提交 CodeGraph DB，再提交另一数据库里的 manifest，跨库仍不原子（需要分布式事务或单库合并）。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-2/P0-3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
| G12 | Durable Staging（JSONL + fsync） | DS/EW | ✅ 已实现 | durable_staging.py |
| G13 | Metrics 收集器 + Prometheus 导出 + 跨进程共享（批次6） | DS | 🟡 复审整改（2026-07-21 批次33） | Python daemon 单例有 `MetricsCollector` + `measure_rpc` + `metrics.snapshot`/`metrics.prometheus` RPC + CLI `cw daemon metrics`。**复审回退（2026-07-21）**：Python daemon 单例有指标，但 Linux systemd unit 启动的是 Rust `cw_daemon`，Rust daemon 无指标埋点。**P1-6 复审整改（2026-07-21 批次33）**：能力区分已明确——**Python daemon（已实现）** vs **Rust system daemon（未实现）**，详见 [docs/design/implementation-status.md](docs/design/implementation-status.md) §P1-6 daemon 能力区分矩阵。结论：不能用 Python 单例证明 Rust 服务具备相同能力 |
| G14 | Health Check endpoint | DS | 🟡 复审整改（2026-07-21 批次33） | Python daemon `daemon_server.py` `__init__` 实例化 `HealthChecker`，`health` RPC 调用 `check_all()` 执行四项检查。**复审回退（2026-07-21）**：同 G13，Python daemon 有 health check，但 Linux systemd 启动 Rust `cw_daemon`，Rust daemon RPC endpoint 只返基础统计并固定 `status=ok`，未执行声称的四项健康检查。**P1-6 复审整改（2026-07-21 批次33）**：能力区分已明确——**Python daemon（已实现）** vs **Rust system daemon（未实现）**，详见 [docs/design/implementation-status.md](docs/design/implementation-status.md) §P1-6 daemon 能力区分矩阵 |
| G15 | Schema Migrator | DS | 🟡 复审整改（2026-07-21 批次33） | Python daemon `daemon_server.py` `__init__` 加载 `DaemonConfig` + `_run_startup_migrations()` 调用 `migrate_daemon_dbs` 对 registry.db / audit.db 执行版本化迁移。**复审回退（2026-07-21）**：Python daemon 有版本化迁移，但 Linux systemd 启动 Rust `cw_daemon`，Rust 端只做 schema-check/init，不是版本化迁移。**P1-6 复审整改（2026-07-21 批次33）**：能力区分已明确——**Python daemon（已实现）** vs **Rust system daemon（未实现）**，详见 [docs/design/implementation-status.md](docs/design/implementation-status.md) §P1-6 daemon 能力区分矩阵 |
...
| G28 | SnapshotManagerService 完整查询（8 方法 + QueryBudget） | E2E | 🟡 复审整改（2026-07-21 批次33） | query_callers/callees/search/symbol/chain/topo/cycles/stats。**复审整改（2026-07-21 批次33）**：8 方法存在但 budget 接入残缺——(a) `query_symbol` 不接 budget 参数；(b) `query_call_chain_down` / `query_detect_cycles` 调用 `b.start()` 但从不调用 `b.visit_node()`，`max_nodes`/`timeout_ms` 形同虚设；(c) `search_symbols` 用 `b.max_results` 作 limit 但不调用 `truncate_results()`；(d) `query_callers`/`query_callees`/`query_topological_order` 仅后置 `truncate_results()`，未传播 budget 到 Rust 端（详见 [docs/design/implementation-status.md](docs/design/implementation-status.md) §P1-4） |
| G29 | Frontier budget（仅 `compute_frontier_with_budget`，通用 query budget 未实现） | E2E | 🟡 复审整改（2026-07-21 批次33） | `rust_ext/src/daemon/budget.rs` `QueryBudget` + `BudgetTracker` + `compute_frontier_with_budget()`，8 单元测试覆盖。**复审回退（2026-07-21）**：`QueryBudget` 只接入 `FrontierComputer::compute_frontier_with_budget`。常用 daemon query（search、callers/callees、call-chain、topological、cycle 等）没有统一 max-results/max-depth/timeout/truncate 执行器。G29 当前标题范围过大。**P1-4 复审整改（2026-07-21 批次33）**：标题已收紧为「Frontier budget」明确范围；详细缺口（详见 [docs/design/implementation-status.md](docs/design/implementation-status.md) §P1-4）：(1) Rust/Python `QueryBudget` 字段不对齐（Rust 3 字段 max_depth/max_nodes/timeout_ms vs Python 5 字段多 max_results/frontier_limit）；(2) Rust daemon 6 个查询 RPC（query.search/callers/callees/call_chain_down/topological_order/detect_cycles）完全未接 `BudgetTracker`，`limit`/`max_depth` 是独立硬编码参数，无 `timeout_ms`/`max_results`/`truncate`；(3) Python daemon 只实现 5/8 个查询 RPC（缺 call_chain_down/topological_order/detect_cycles）；(4) Python `SnapshotManagerService` budget 接入残缺——`query_call_chain_down`/`query_detect_cycles` 调用 `b.start()` 但从不调用 `b.visit_node()`，`max_nodes`/`timeout_ms` 形同虚设；(5) MCP 工具层（`search_symbols`/`get_callers`/`get_callees`/`get_topological_order`/`get_impact`/`get_call_chain_down`/`detect_cycles`）完全不暴露 `timeout`/`truncate` 参数；(6) `DaemonClient` 不通过 wire protocol 传递 budget |
...
| # | 问题 | 详情 | 需更新文件 |
|---|------|------|------------|
| I1 | ✅ 已修复（2026-07-22 P1-5） | **复审回退（2026-07-21）**：源码实算 206 MCP / 35 功能 Mixin（另有 `CodeGraphBase`）/ 40 db_*.py / v41 / 16 语言。旧同步至 205/33/40 与源码不符。**P1-5 整改（2026-07-22 批次34）**：大部分文档已统一至 206/35/40/v41。**复审整改-v2 回退（2026-07-22 批次35）**："76 文档零不一致"声明为假——`docs/architecture.md:49,323` 仍写"39 个文件"。**P1-5 v2 闭合（2026-07-22）**：`check_baseline.py` 三个扫描盲区已闭合——(1) `db_files` 新增 `reverse_pattern` 捕获 `db_*.py`（N 个文件）`反向格式（兼容全角/半角括号）；(2) 移除 `→` 从 `SKIP_MARKERS`，改为在 `→` 处分段检查当前值；(3) `scan_document_consistency` 返回 `(inconsistencies, scanned_count)`，报告实际扫描数 53/77。`docs/architecture.md` L49/L323 的 39→40、`_feature_matrix.md` L362 I38 的 33→35/39→40 已修复。7 个新测试覆盖三个盲区。 | IS, RM, MCT, ARC |
| I2 | ✅ 已修复（2026-07-22 P1-2） | CA 表格已改"未暴露"为"✅ 已暴露"。**复审回退（2026-07-21）**：D7 跨仓库影响传播只修复 `target_symbol_hash` 空字符串，仍按 import 尾段匹配同名符号，`Dict[name]` 覆盖重名，`cross_repo_deps` 无唯一约束。**P1-2 复审整改（2026-07-21 批次32）**：算法已修复（FQN 三级匹配 + UNIQUE 索引 + INSERT OR IGNORE 幂等）。**P1-2 v2 闭合（2026-07-22）**：`cross_repo_impact()` 方向修正 + 跨 workspace 去重 key + 短名匹配恒真条件移除，D7 已升级为 ✅，CA 表格状态一致。 | CA |
...
| I17 | ✅ 已修复（2026-07-22 P1-5） | **复审回退（2026-07-21）**：Schema v37→v40 同步可保留，但 205/33 与源码 206/35 仍冲突。**P1-5 整改（2026-07-22 批次34）**：大部分文档已统一至 206/35/40/v41。**复审整改-v2 回退（2026-07-22 批次35）**："76 文档零不一致"声明为假——`docs/architecture.md:49,323` 仍写"39 个文件"。**P1-5 v2 闭合（2026-07-22）**：`check_baseline.py` 三个扫描盲区已闭合（reverse_pattern + → 分段 + 实际扫描数），`docs/architecture.md` L49/L323 和 `_feature_matrix.md` L362 I38 的 39→40 已修复。7 个新测试覆盖三个盲区。 | ARC, IS, README.md, UG |
...
| I19 | ✅ 已修复（2026-07-22 P1-2） | D1/D7 评审修正：D1 "🟡 部分完成（BLOB + Rust/numpy）"。**复审回退（2026-07-21）**：D7 状态从 ✅ 回退为 🟡，仅修复 `target_symbol_hash` 空字符串，跨仓库检测算法仍有缺陷。**P1-2 复审整改（2026-07-21 批次32）**：算法已修复（FQN 三级匹配 + UNIQUE 索引 + INSERT OR IGNORE 幂等）。**P1-2 v2 闭合（2026-07-22）**：短名匹配 0.7 分支已可达 + `cross_repo_impact()` 方向已修正，D7 状态升级为 ✅。 | _feature_matrix.md D1/D7 |
...
| I22 | ✅ 已修复（2026-07-22 P1-1） | A19/A21 PR 检查：PRChecker 改用 `check_before_edit` + 异常上浮 + SARIF `executionNotifications`。**复审回退（2026-07-21）**：`passed = errors == 0` 未纳入 `run_errors`/`scan_complete`，`_query_open_findings` 只查 `guardrail_findings` 未合并 `semgrep_findings`，GitHub Action 仍 exit 0。**P1-1 闭合（2026-07-22）**：三条 fail-open 路径已闭合（git diff 失败抛 `GitDiffError` + Semgrep `success=false` 检查 + SQL 异常记录到 `run_errors`），`passed = (errors == 0) and scan_complete`，Semgrep SQL 添加 `workspace_id` 过滤。A19/A21 状态升级为 ✅。16 测试全部通过。 | _feature_matrix.md A19/A21 |
...
| J8 | Daemon UDS 协议闭合 | ❌ 未闭合（2026-07-22 复审整改-v2 回退） | Rust 端 SnapshotDaemonState 已实现 36 个 handle_xxx；Python daemon_client 三个高级查询方法补齐 RPC 路径；CLI 子命令齐全。**复审回退（2026-07-21）→部分修复**：(1) Rust `memfd.rs` 无 `F_GET_SEALS` 也无 owner UID 校验→**P1-3 已修复**（六重校验）；(2) `daemon_handle_refresh` 只更新 CAS/generation，未写 `workspace_manifests` 也未对 symbols/calls 执行 delta apply→**P0-1 Python 路径已修复**（新增 `db_cas_merge.py` + `upsert_manifest`）；(3) `codegraph_db_path_template` 默认空→**P0-1 已修复**（dispatch 层显式传入）；(4) workspace_id 级 ACL 缺口→**Python 端已修复**（5 个 handler 接入 `_owned_workspace_by_id`），**Rust 端 5 个 handler 仍未修复**（[snapshot_state.rs:960-1149](file:///c:/git_work/callwarden/rust_ext/src/daemon/snapshot_state.rs#L960) 忽略 `_peer`）；(5) dispatch 层 `int(workspace_id)` bug→**已修复**。**P0-2 整改（2026-07-22）**：Python 路径 step 4 后置 + step 2 stale_seq 放宽 + symbol_contents/callee_id 修复，8 测试通过；Rust daemon 路径降级为 CAS-only（已在 replicator.rs + systemd unit 加文档说明，企业部署需用 Python daemon）。**未修复**：Rust system daemon 的 workspace_id ACL 仍未闭合（Rust 端 5 个 handler 仍忽略 _peer）。**复审整改-v2 回退（2026-07-22）**：纵向协议链未闭合——Rust system daemon workspace_id ACL 未闭合 + Rust daemon 路径降级为 CAS-only（save-to-query 数据链未闭合）+ Python 路径跨库不原子 + snapshot 混入其他 workspace 数据。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| K4 | daemon dispatch 未接入 | ❌ 未闭合（2026-07-22 复审整改-v2 回退） | **已修复（2026-07-21 P0-1 整改）**：`daemon_server.py` `workspace.file.refresh` 调用 `daemon_handle_refresh` + staging + replicate 存在；dispatch 层 `int(workspace_id)` bug 修复为 `int(workspace["workspace_id"])`；`daemon_handle_refresh` CAS committed 后新增 step 5 调用 `merge_cas_to_codegraph` + `upsert_manifest`；`codegraph_db_path_template` 默认空问题通过 `res["codegraph_db_path"]` 显式传入绕过。**P0-2 整改（2026-07-22）**：Python 路径 step 4 后置 + step 2 stale_seq 放宽 + symbol_contents/callee_id 修复，8 测试通过；Rust daemon 路径降级为 CAS-only（已在 `replicator.rs:740-769` publish_snapshot + `daemon_handle_refresh` 函数注释 + systemd unit 文件三处加文档说明）。企业部署 save-to-query 数据链需用 Python daemon。**复审整改-v2 回退（2026-07-22）**：dispatch 层 Python 路径已接入，但 systemd 启动的是 Rust daemon 不是 Python daemon，企业部署 save-to-query 数据链实际未闭合（纵向协议链未闭合）。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-2](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| N5 | Windows WiX MSI（x64/arm64 + Authenticode） | CP | 🟡 部分修复（2026-07-22 P0-3 PyInstaller） | **P0-3 整改（2026-07-22）**：Gate 4a 新增 PyInstaller --onedir 构建步骤（安装 wheel[all] + pyinstaller，运行 callwarden.spec），产出到 `release/dist/windows/cw/` 等目录；callwarden.wxs 更新为引用 --onedir 目录结构（`BuildOutputDir` 指向 `release/dist/windows/cw/`，用 heat harvest `_internal/` 目录）；`ClientBuildOutputDir` 指向 `release/dist/windows/cw-client/`。**剩余**：Authenticode 签名仍仅注释命令；heat harvest 步骤已在 enterprise-release.yml 中添加（`wix heat dir` 生成 PyInstallerInternal ComponentGroup，安装到 InternalFolder）。 |
| N6 | macOS universal2 pkg + notarization | CP | 🟡 部分修复（2026-07-22 P0-3 PyInstaller） | **P0-3 整改（2026-07-22）**：`build_pkg.sh` 改用 PyInstaller --onedir 打包自包含二进制（含 Python 解释器 + 全部依赖 + Rust 扩展），不再依赖系统 Python；`enterprise-release.yml` Gate 2 macOS `rust_target` 从无效的 `universal2-apple-darwin` 改为 `aarch64-apple-darwin`（build_pkg.sh 用 lipo 合并 x86_64 + aarch64）。**复审整改-v2 回退（2026-07-22）**：PyInstaller 改造仅修改构建脚本，未经 runner 实际验证，复审报告 §3 P0-4 判定安装产物确定不可运行（不能证明 pkg 或 11 gate 已闭合）。**剩余**：notarization 仍仅注释命令；CW_BUILD_UNSIGNED 支持存在。 |
| N7 | Linux deb 5 子包 + tar.zst（RPM 不在发布范围） | CP | 🟡 部分修复（2026-07-22 P0-3 PyInstaller） | **P0-3 整改（2026-07-22）**：`build_packages.sh` 改用 PyInstaller --onedir 打包自包含二进制，不再从 venv 复制 console_scripts；各子包把 --onedir 目录复制到 `/usr/lib/callwarden/runtime/` 并在 `/usr/bin/` 创建软链接；control 文件添加 `libstdc++6` 依赖（PyInstaller 产物动态链接 libstdc++）；maintainer scripts 修正不存在的 cw-daemon 子命令调用（`drain`→systemctl stop，`snapshot create`→tar 备份，`migrate`→schema-check --strict，`--pre-upgrade`→--strict）。**复审整改-v2 回退（2026-07-22）**：PyInstaller 改造仅修改构建脚本，未经 runner 实际验证，复审报告 §3 P0-4 判定安装产物确定不可运行。 |
| N8 | Release CI enterprise-release.yml（批次5 文档对齐） | CP | 🟡 部分修复（2026-07-22 P0-3 PyInstaller） | **P0-3 整改（2026-07-22）**：Gate 4a Windows 新增 PyInstaller --onedir 构建步骤替换原 fail-fast exit 1；Gate 2 macOS `rust_target` 修正为 `aarch64-apple-darwin`；Gate 4a/4b/4c 三平台均改用 PyInstaller 产出自包含二进制。**复审整改-v2 回退（2026-07-22）**：11 gate 当前必在 Windows Gate 4a 失败（PyInstaller 改造仅修改构建脚本，未经 runner 实际验证，复审报告 §3 P0-4 判定安装产物确定不可运行）。**剩余**：实际 runner E2E 验证待 CI 触发。heat harvest 步骤已在 enterprise-release.yml 中添加（`wix heat dir` 生成 `_internal.wxs`，`wix build` 同时编译两个 wxs 文件）；Gate 4a needs 添加 gate1-source-tests 以访问 version 输出。 |
