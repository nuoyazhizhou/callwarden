...
| G8 | Watcher Generation 状态机 | WG/EW | ❌ 复审回退（2026-07-22 批次35） | session/generation CAS 和 CAS publish 存在；canonical_bytes 协议 P0-2 已修复。**P0-1 整改（2026-07-21）**：Python `daemon_handle_refresh` CAS committed 后调用 `merge_cas_to_codegraph` + `upsert_manifest`。**复审整改-v2 回退（2026-07-22 批次35）**：(1) Rust daemon `publish_snapshot` 只调用 `build_and_publish_blocking` 重载 SQLite（[replicator.rs:740-768](file:///c:/git_work/callwarden/rust_ext/src/daemon/replicator.rs#L740)），未做 CAS → CodeGraph merge；Linux systemd 启动 Rust daemon，真实企业部署 save-to-query 未闭合。(2) Python 路径非原子提交：先提交 `latest_committed_generation`（[replicator.py:254-269](file:///c:/git_work/callwarden/server/replicator.py#L254)），后做 merge/manifest；后半段失败后同一 seq 重试被判 stale 丢弃。(3) 共享 GraphStore 无 `workspace_id` 过滤，snapshot 可能混入其他工作区符号。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-2](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| G11 | Replicator（CAS → Manifest → Snapshot） | DS/EW | ❌ 复审回退（2026-07-22 批次35） | `DaemonConfig.codegraph_db_path_template` 默认空字符串问题通过 dispatch 层从 `res["codegraph_db_path"]` 显式传入绕过。**P0-1 整改（2026-07-21）**：Python `daemon_handle_refresh` CAS committed 后新增 step 5——调用 `merge_cas_to_codegraph` + `upsert_manifest`。**复审整改-v2 回退（2026-07-22 批次35）**：(1) Rust daemon `publish_snapshot` 未做 CAS merge（[replicator.rs:740-768](file:///c:/git_work/callwarden/rust_ext/src/daemon/replicator.rs#L740)），Linux systemd 启动 Rust daemon，真实企业部署 save-to-query 未闭合。(2) Python `merge_cas_to_codegraph` 单独提交 CodeGraph DB，再提交另一数据库里的 manifest，跨库不原子。(3) 合并器只写 `file_contents`/`symbols`/`calls`，未写 `symbol_contents`/版本事实；`symbols.symbol_hash` 外键指向 `symbol_contents.content_hash` 会断。(4) 只删除旧 caller 出边，不清理指向被替换 symbol 的入边。(5) 以 `callee_id=0` 保存未解析调用。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-2/P0-3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| K4 | daemon dispatch 未接入 | ❌ 复审回退（2026-07-22 批次35） | **已修复（2026-07-21 P0-1 整改）**：`daemon_server.py` `workspace.file.refresh` 调用 `daemon_handle_refresh` + staging + replicate 存在；dispatch 层 `int(workspace_id)` bug 修复为 `int(workspace["workspace_id"])`；`daemon_handle_refresh` CAS committed 后新增 step 5 调用 `merge_cas_to_codegraph` + `upsert_manifest`；`codegraph_db_path_template` 默认空问题通过 `res["codegraph_db_path"]` 显式传入绕过。**复审整改-v2 回退（2026-07-22 批次35）**：上述修复仅覆盖 Python daemon 路径。Linux systemd 启动 Rust `cw_daemon`，Rust daemon `publish_snapshot` 未做 CAS → CodeGraph merge（[replicator.rs:740-768](file:///c:/git_work/callwarden/rust_ext/src/daemon/replicator.rs#L740)），真实企业部署 save-to-query 数据链未闭合。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-2](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
