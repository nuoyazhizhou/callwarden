...
| # | 问题 | 详情 | 需更新文件 |
|---|------|------|------------|
| I1 | 🟡 部分修复（2026-07-22 批次35） | **复审回退（2026-07-21）**：源码实算 206 MCP / 35 功能 Mixin（另有 `CodeGraphBase`）/ 40 db_*.py / v41 / 16 语言。旧同步至 205/33/40 与源码不符。**P1-5 整改（2026-07-22 批次34）**：大部分文档已统一至 206/35/40/v41。**复审整改-v2 回退（2026-07-22 批次35）**："76 文档零不一致"声明为假——`docs/architecture.md:49,323` 仍写"39 个文件"（扫描器只匹配"N 个 db_*.py"，抓不到反引号包裹的 `db_*.py`（39 个文件）`）；`_feature_matrix.md:362` I38 仍写"33 个 Mixin 类（39 个 db_*.py 文件）"（因含 `→` 被整行跳过）。基线值正确但全文档一致性声明应撤回。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §4 P1-5](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) | IS, RM, MCT, ARC |
...
| I17 | 🟡 部分修复（2026-07-22 批次35） | **复审回退（2026-07-21）**：Schema v37→v40 同步可保留，但 205/33 与源码 206/35 仍冲突。"全部统一为 205/33"声明为假。**P1-5 整改（2026-07-22 批次34）**：大部分文档已统一至 206/35/40/v41。**复审整改-v2 回退（2026-07-22 批次35）**："76 文档零不一致"声明为假——`docs/architecture.md:49,323` 仍写"39 个文件"；`_feature_matrix.md:362` I38 仍写"33/39"。基线值正确但全文档一致性声明应撤回。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §4 P1-5](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) | ARC, IS, README.md, UG |
...
| N6 | macOS universal2 pkg + notarization | CP | ❌ 复审回退（2026-07-22 批次35） | **P0-3 整改（2026-07-21 批次31）**：`build_pkg.sh` 删除 placeholder 生成逻辑，改为从 wheel 提取 console_scripts；新增 `CW_BUILD_UNSIGNED` 支持；workflow Gate 4b 环境变量 `APPLE_*` → `CW_APPLE_*` 对齐。**复审整改-v2 回退（2026-07-22 批次35）**：包内只复制临时 venv 的 launcher（[build_pkg.sh:164-190](file:///c:/git_work/callwarden/release/macos/build_pkg.sh#L164)），没有打包 Python 解释器、site-packages 和依赖，安装后 `cw`/`cw-client`/`cw-agent` 无法启动。Gate 2 还声明无效 Rust target `universal2-apple-darwin`（[enterprise-release.yml:108](file:///c:/git_work/callwarden/.github/workflows/enterprise-release.yml#L108)），实际 cargo 命令未使用 matrix target。这不是"仅待 runner 验证"，而是确定性失败。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-4](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
| N7 | Linux deb 5 子包 + tar.zst（RPM 不在发布范围） | CP | ❌ 复审回退（2026-07-22 批次35） | **P0-3 整改（2026-07-21 批次31）**：`build_packages.sh` 从 wheel 提取 console_scripts；Cargo.toml/systemd/build_packages.sh 三方命名统一；新增 `--offline-bundle-only` flag；workflow Gate 4c 删除 `\|\| true`。**复审整改-v2 回退（2026-07-22 批次35）**：包内只复制临时 venv 的 launcher（[build_packages.sh:169-195](file:///c:/git_work/callwarden/release/linux/build_packages.sh#L169)），shebang 指向构建机临时 venv，但包中没有该 venv、Python 模块或依赖；control 文件甚至只依赖 libc/libgcc。安装后 `cw`/`cw-client`/`cw-agent` 无法启动。Linux maintainer scripts 调用 Rust CLI 不存在的命令（`drain`/`snapshot create`/`migrate`/`schema-check --pre-upgrade`）。这不是"仅待 runner 验证"，而是确定性失败。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-4](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
| N8 | Release CI enterprise-release.yml（批次5 文档对齐） | CP | 🟡 复审回退（2026-07-22 批次35） | **P0-3 整改（2026-07-21 批次31）**：Gate 1 `version.toml` key 修正；Gate 3 `parse_file_lang` 签名错误改为 `parse_canonical_bytes_py`；Gate 3 新增 `cw --version` 黑盒命令；Gate 4a Windows MSI PyInstaller exe fail-fast；Gate 4b macOS `APPLE_*` → `CW_APPLE_*`；Gate 4c Linux 删除 `\|\| true`。**复审整改-v2 回退（2026-07-22 批次35）**：11 gate 当前必在 Windows Gate 4a 失败（[enterprise-release.yml:239-250](file:///c:/git_work/callwarden/.github/workflows/enterprise-release.yml#L239) 明确因缺 PyInstaller 的 `cw.exe`/`cw-client.exe`/`runtime/python.exe` 而 `exit 1`）。Linux/macOS 包确定不可运行（同 N6/N7）。这不是"未运行过 E2E"，而是 gate 必然失败。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-4](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
