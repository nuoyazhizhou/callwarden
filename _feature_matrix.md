...
| # | 功能点 | 来源 | 状态 | 备注 |
|---|--------|------|------|------|
| G1 | 三层存储（Global CAS / Toolchain / Thin Workspace） | EA/DS | ✅ 已修复（2026-07-22 P0-1 Rust daemon ACL 闭合） | CAS/toolchain/workspace 三层存在。**P0-1 整改**：ADMIN_ONLY_METHODS + is_admin 顶层 fail-closed。**P0-2 整改（2026-07-21）**：Python 端 5 个 handler 接入 `_owned_workspace_by_id` workspace owner ACL。**P0-1 Rust daemon ACL 闭合（2026-07-22 commit 498c8ba）**：Rust 5 个 handler（`handle_toolchain_resolve` / `handle_build_context_list` / `handle_resolved_edges_store/get/count`）从 `_peer` 改为 `peer`，调用 `owned_workspace_by_id(&self.base.registry, peer.uid, workspace_id)?` 先鉴权再访问资源。新增 `owned_workspace_by_id` + `get_workspace_by_numeric_id` 函数。14 个跨 UID 拒绝路径测试全部通过。 |
...
| G3 | UDS + SO_PEERCRED 认证 | DS/DI | ✅ 已修复（2026-07-22 P0-1 Rust daemon ACL 闭合） | SO_PEERCRED 认证 + workspace owner 过滤存在。**P0-1 整改**：ADMIN_ONLY_METHODS + is_admin 顶层 fail-closed。**P0-2 整改（2026-07-21）**：Python 端 5 个 handler 接入 `_owned_workspace_by_id` workspace owner ACL；Rust `handle_snapshot_list_workspaces` 按 peer_uid 过滤。**P0-1 Rust daemon ACL 闭合（2026-07-22 commit 498c8ba）**：Rust 5 个 workspace-id handler 接入 `peer.uid` 校验，先鉴权再访问资源。14 测试通过。 |
| G4 | Workspace Registry + Container Mount Mapping | DS/RP | ✅ 已修复（2026-07-22 P0-1 Rust daemon ACL 闭合） | registry 和 mount CRUD 存在。**P0-1 整改**：mount.register/delete 加入 ADMIN_ONLY_METHODS。**P0-2 整改（2026-07-21）**：`mount.list` 加入 ADMIN_ONLY_METHODS。**P0-1 Rust daemon ACL 闭合（2026-07-22 commit 498c8ba）**：workspace_id ACL 已闭合（同 G1/G3）。14 测试通过。 |
