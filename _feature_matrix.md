...
| G8 | Watcher Generation 状态机 | WG/EW | 🟡 部分完成（2026-07-22 第二轮复审回退） | session/generation CAS 和 CAS publish 存在；canonical_bytes 协议已修复。**复审回退（2026-07-22 第二轮）**：完整发布提交点仍错误——(a) generation committed 时序仍可能早于 snapshot publish；(b) `Ok(false)` 并发冲突被忽略；(c) replicate 失败未回滚 committed。**P0-1 v2 整改（2026-07-22）**：committed 移到 replicate 之后；`Ok(false)` 中止 replicate；replicate 失败回滚 committed（`file_generation_uncommit` 新增方法 + 3 测试）。361 daemon 测试通过。**剩余**：实际 runner E2E 验证待 CI 触发；多文件 merge 顺序仍依赖客户端调用顺序。证据：复审报告 `docs/design/feature-matrix-remediation-patch-reaudit-2026-07-22.md` §2 P0-1 |
...
| G11 | Replicator（CAS → Manifest → Snapshot） | DS/EW | ❌ 未完成（2026-07-22 第二轮复审回退） | `DaemonConfig.codegraph_db_path_template` 默认空字符串问题已修复（dispatch 层显式传入）。**复审回退（2026-07-22 第二轮）**：默认 fresh CodeGraph DB 不可用——`Connection::open()` 只创建空 SQLite 文件，merge 时查询 `workspaces` 等表会报 `no such table`，snapshot 链不可用。**P0-2 v2 整改（2026-07-22）**：新增 `init_codegraph_schema()` 用 `CREATE TABLE IF NOT EXISTS` 幂等建表（6 张表 + 15 索引 + FTS5 虚拟表 + 3 触发器），fresh DB 首次打开时调用；workspace.rs 调用点接入。3 个新测试覆盖 fresh DB schema 初始化（幂等 + merge 成功）。361 daemon 测试通过。**剩余**：实际 runner E2E 验证待 CI 触发；CAS merge 跨文件 resolve 仍有顺序依赖（P1-2 v2 已部分修复）。证据：复审报告 `docs/design/feature-matrix-remediation-patch-reaudit-2026-07-22.md` §2 P0-2 |
...
| H14 | 跨平台打包发布（MSI/PKG/DEB） | CP | 🟡 部分修复（2026-07-22 第二轮复审） | **P0-3 整改**：`python release/build.py --wheel` 参数错误已修复；`_ensure_rust_ext_at_root()` 自动复制 Rust 二进制；`MANIFEST.in` 显式声明；`cw --version`/`-V` 已支持；`build_pkg.sh`/`build_packages.sh` 改用 PyInstaller --onedir。**P0-3 v2 + P0-4 v2 + P0-5 v2 整改（2026-07-22）**：(1) Linux 三个包独立 runtime 目录 + registry fresh init schema 可记为完成子项；(2) Windows WiX v4 CLI 改用预装 WiX v3.14（candle/light/heat）避免 v3 WXS 不兼容；(3) macOS arm64-only 降级（不再 lipo 合成 universal2）+ Mach-O 架构校验。**剩余**：三平台未在干净 runner E2E 验证；socket 权限改为 fail-closed（P0-3 v2）；跨平台 errno 用 `io::Error::last_os_error()` 替代 `libc::__errno_location()`。证据：复审报告 `docs/design/feature-matrix-remediation-patch-reaudit-2026-07-22.md` §2 P0-3/P0-4/P0-5 |
...
| J8 | Daemon UDS 协议闭合 | 🟡 部分完成（2026-07-22 第二轮复审回退） | Rust 端 SnapshotDaemonState 已实现 36 个 handle_xxx；Python daemon_client 三个高级查询方法补齐 RPC 路径；CLI 子命令齐全。**复审回退（2026-07-22 第二轮）**：RPC/ACL 组件存在，但 save-to-query 生产链未闭合——fresh CodeGraph DB schema 未初始化、CAS merge 跨文件 resolve 顺序依赖、manifest 元数据错误（详见 G8/G11 回退）。**P0-1 v2 + P0-2 v2 + P1-2 v2 整改（2026-07-22）**：committed 时序修复（`file_generation_uncommit`）、`init_codegraph_schema()` 幂等建表、`resolve_unresolved_calls_in_workspace()` workspace 级回扫 pass、manifest file_size/total_lines 分离。361 daemon 测试通过（含 4 个新增 P1-2 v2 测试）。**剩余**：实际 runner E2E 验证待 CI 触发。证据：复审报告 `docs/design/feature-matrix-remediation-patch-reaudit-2026-07-22.md` §2 + §3 P1-2 |
...
| K4 | daemon dispatch 未接入 | 🟡 部分完成（2026-07-22 第二轮复审回退） | **已修复（2026-07-21 P0-1 整改）**：`daemon_server.py` `workspace.file.refresh` 调用 `daemon_handle_refresh` + staging + replicate 存在；dispatch 层 `int(workspace_id)` bug 修复；`daemon_handle_refresh` CAS committed 后新增 step 5 调用 `merge_cas_to_codegraph` + `upsert_manifest`；`codegraph_db_path_template` 默认空问题通过 dispatch 层显式传入绕过。**复审回退（2026-07-22 第二轮）**：企业部署 save-to-query 数据链未闭合——(a) generation committed 时序仍可能早于 snapshot publish；(b) fresh CodeGraph DB schema 未初始化；(c) CAS merge 跨文件 resolve 顺序依赖；(d) manifest file_size/total_lines 混淆。**P0-1 v2 + P0-2 v2 + P1-2 v2 整改（2026-07-22）**：所有四个问题已修复（详见 G8/G11 回退说明），361 daemon 测试通过。**剩余**：实际 runner E2E 验证待 CI 触发；socket 权限已改为 fail-closed（P0-3 v2）。证据：复审报告 `docs/design/feature-matrix-remediation-patch-reaudit-2026-07-22.md` §2 + §3 P1-2 |
...
| N6 | macOS arm64 pkg + notarization（原 universal2，2026-07-22 降级） | CP | 🟡 部分修复（2026-07-22 P0-5 v2 arm64-only 降级） | **P0-3 整改（2026-07-22）**：`build_pkg.sh` 改用 PyInstaller --onedir 打包自包含二进制；`enterprise-release.yml` Gate 2 macOS `rust_target` 改为 `aarch64-apple-darwin`。**复审回退（2026-07-22 第二轮）**：原声称 universal2 实际只有 arm64——PyInstaller runtime 是宿主架构（macos-latest=arm64），lipo 合成的 universal2 Rust 扩展与入口/嵌入式 Python 架构不匹配。**P0-5 v2 整改（2026-07-22）**：诚实降级为 arm64-only——(1) 只构建 `aarch64-apple-darwin` Rust 扩展（不再 lipo 合成）；(2) `lipo -archs` 校验所有 Mach-O 产物架构（cw/cw-client/cw-agent + 嵌入式 Python）；(3) 产物文件名标记 arm64；(4) `version.toml` `macos = ["arm64"]`。**剩余**：notarization 仍仅注释命令；CW_BUILD_UNSIGNED 支持存在；x86_64 支持需 macos-13 runner（未来扩展）。证据：复审报告 `docs/design/feature-matrix-remediation-patch-reaudit-2026-07-22.md` §2 P0-5 |
