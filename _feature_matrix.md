...
| G3 | UDS + SO_PEERCRED 认证 | DS/DI | ✅ 已修复（2026-07-22 P0-1 Rust daemon ACL） | SO_PEERCRED 认证 + workspace owner 过滤存在。**P0-1 整改**：ADMIN_ONLY_METHODS + is_admin 顶层 fail-closed。**P0-2 整改（2026-07-21）**：Python 端 5 个 handler 接入 `_owned_workspace_by_id` workspace owner ACL；Rust `handle_snapshot_list_workspaces` 按 peer_uid 过滤。**复审整改-v2 回退（2026-07-22 批次35）**：Rust system daemon 的 5 个 workspace-id handler 仍忽略 `_peer`（[snapshot_state.rs:960-1149](file:///c:/git_work/callwarden/rust_ext/src/daemon/snapshot_state.rs#L960)）。Python ACL 修复不能代表 Rust daemon 闭合。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
| G4 | Workspace Registry + Container Mount Mapping | DS/RP | ✅ 已修复（2026-07-22 P0-1 Rust daemon ACL） | registry 和 mount CRUD 存在。**P0-1 整改**：mount.register/delete 加入 ADMIN_ONLY_METHODS。**P0-2 整改（2026-07-21）**：`mount.list` 加入 ADMIN_ONLY_METHODS。**复审整改-v2 回退（2026-07-22 批次35）**：Rust system daemon 的 workspace_id ACL 仍未闭合（同 G1/G3）。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| G8 | Watcher Generation 状态机 | WG/EW | 🟡 部分修复（2026-07-22 P0-2 Python + Rust 降级文档） | session/generation CAS 和 CAS publish 存在；canonical_bytes 协议 P0-2 已修复。**P0-1 整改（2026-07-21）**：Python `daemon_handle_refresh` CAS committed 后调用 `merge_cas_to_codegraph` + `upsert_manifest`。**P0-2 整改（2026-07-22）Python 路径**：(1) step 4 (committed_generation) 移到 step 5 (merge/manifest) 之后，避免后半段失败后重试被判 stale 丢弃；(2) step 2 stale_seq 逻辑放宽——同 seq 但 `latest_committed_generation` 为空时允许重试；(3) `merge_cas_to_codegraph` 补 symbol_contents UPSERT + callee_id 入边清理。8 测试通过。**P0-2 整改（2026-07-22）Rust daemon 路径降级为 CAS-only**：`publish_snapshot` 仅重载 SQLite 不做 CAS merge（[replicator.rs:740-769](file:///c:/git_work/callwarden/rust_ext/src/daemon/replicator.rs#L740) 已加文档注释），`daemon_handle_refresh` 函数注释明确 CAS-only 路径（无 step 5 merge），systemd unit 文件注释说明企业部署需改用 Python daemon。详见 [feature-matrix-remediation-reaudit-2026-07-22.md §3 P0-2](docs/design/feature-matrix-remediation-reaudit-2026-07-22.md) |
...
| N6 | macOS universal2 pkg + notarization | CP | 🟡 部分修复（2026-07-22 P0-3 PyInstaller） | **P0-3 整改（2026-07-22）**：`build_pkg.sh` 改用 PyInstaller --onedir 打包自包含二进制（含 Python 解释器 + 全部依赖 + Rust 扩展），不再依赖系统 Python；`enterprise-release.yml` Gate 2 macOS `rust_target` 从无效的 `universal2-apple-darwin` 改为 `aarch64-apple-darwin`（build_pkg.sh 用 lipo 合并 x86_64 + aarch64）。**剩余**：notarization 仍仅注释命令；CW_BUILD_UNSIGNED 支持存在。 |
| N7 | Linux deb 5 子包 + tar.zst（RPM 不在发布范围） | CP | 🟡 部分修复（2026-07-22 P0-3 PyInstaller） | **P0-3 整改（2026-07-22）**：`build_packages.sh` 改用 PyInstaller --onedir 打包自包含二进制，不再从 venv 复制 console_scripts；各子包把 --onedir 目录复制到 `/usr/lib/callwarden/runtime/` 并在 `/usr/bin/` 创建软链接；control 文件添加 `libstdc++6` 依赖（PyInstaller 产物动态链接 libstdc++）；maintainer scripts 修正不存在的 cw-daemon 子命令调用（`drain`→systemctl stop，`snapshot create`→tar 备份，`migrate`→schema-check --strict，`--pre-upgrade`→--strict）。 |
| N8 | Release CI enterprise-release.yml（批次5 文档对齐） | CP | 🟡 部分修复（2026-07-22 P0-3 PyInstaller） | **P0-3 整改（2026-07-22）**：Gate 4a Windows 新增 PyInstaller --onedir 构建步骤替换原 fail-fast exit 1；Gate 2 macOS `rust_target` 修正为 `aarch64-apple-darwin`；Gate 4a/4b/4c 三平台均改用 PyInstaller 产出自包含二进制。**剩余**：实际 runner E2E 验证待 CI 触发。heat harvest 步骤已在 enterprise-release.yml 中添加（`wix heat dir` 生成 `_internal.wxs`，`wix build` 同时编译两个 wxs 文件）；Gate 4a needs 添加 gate1-source-tests 以访问 version 输出。 |
